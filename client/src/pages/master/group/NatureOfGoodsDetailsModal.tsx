...
export default function NatureOfGoodsDetailsModal({ isOpen, onClose, companyId }: NatureOfGoodsDetailsModalProps) {
  ...
  const handleCreateNature = async () => {
    ...
    try {
      ...
      if (res.success && res.tcsNatureOfGoods) {
        setNatureList((prev) => [...prev, res.tcsNatureOfGoods]);
        setSelectedNature(res.tcsNatureOfGoods.name || newNatureName.trim());
        setShowCreateForm(false);
        setNewNatureName("");
      }
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white border border-zinc-200 rounded shadow-xl w-[520px] max-h-[80vh] flex flex-col">
        <div className="px-4 py-3 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
          <span className="text-sm font-semibold text-zinc-800">Nature of Goods</span>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-700 text-lg font-bold">&times;</button>
        </div>

        <div className="p-4">
          <div className="flex items-center gap-2 mb-4">
            <span className="text-sm text-zinc-600 w-40">Nature of Goods</span>
          ...
          {showCreateForm && (
            <div className="border rounded p-4 mt-2 bg-zinc-50">
              <div className="text-sm font-semibold text-zinc-800 mb-3">Create Nature of Goods</div>
              ...
              <div className="flex items-center gap-2 mb-3 ml-4">
                <span className="text-sm text-zinc-600 w-28">Without PAN</span>
                <span className="text-zinc-400 mr-2">:</span>
                <input className={inputCls} type="number" defaultValue="0" placeholder="0 %" />
                <span className="text-sm text-zinc-500">%</span>
              </div>
              <div className="text-xs font-semibold text-zinc-700 mb-2">Rate for other collectee types</div>
              <div className="flex items-center gap-2 mb-3 ml-4">
                <span className="text-sm text-zinc-600 w-28">With PAN</span>
                <span className="text-zinc-400 mr-2">:</span>
                <input className={inputCls} type="number" defaultValue="0" placeholder="0 %" />
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm text-zinc-600 w-32">Tax calculation based on realisation</span>
                <span className="text-zinc-400 mr-2">:</span>
                <span className="text-sm py-1">No</span>
              </div>
              <div className="flex items-center gap-2 mb-3">
