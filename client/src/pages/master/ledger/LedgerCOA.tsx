...
export default function LedgerCOA() {
    ...
    (async () => {
      try {
        ...
        if (treeRes.success) {
          ...
          // By default, expand top-level groups
  ...
  // Recursively flatten tree to build O(1) group name lookup map
  ...
  // Group ledgers by group_id for tree lookup
  ...
  // Filtered Flat Ledgers list
  const flatLedgerList = useMemo(() => {
    let list = ledgers.map((l) => {
      const parentName = l.group_id ? (groupMap[l.group_id] || "Primary") : "Primary";
      return {
        ...l,
    ...
    if (showUnusedOnly) {
      list = list.filter((l) => Number(l.opening_balance) === 0);
    }
  ...
  // Helper to expand all groups in the tree
  const expandAll = () => {
  ...
  // Helper to collapse all groups in the tree
  const collapseAll = () => {
    setExpandedGroups({});
    setExpandedLedgers({});
  };
  ...
  const renderTreeNode = (node: TreeNode, depth: number) => {
    ...
    // In unused mode, only display matching ledgers
                    ...
                    <div className="flex-1 flex items-center justify-between pr-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/master/alter/ledger`, { state: { ledgerId } });
                          }}
