...
const TAX_TYPE_META: { key: TaxType; label: string; list: string[]; listLabel: string }[] = [
  { key: 'gst', label: 'GST', list: GST_CLASSIFICATIONS, listLabel: 'List of Classifications' },
  {
    key: 'service_tax',
    label: 'Service Tax',
    list: SERVICE_TAX_CLASSIFICATIONS,
    listLabel: 'List of Classifications',
  },
  {
    key: 'excise',
    label: 'Excise',
    list: EXCISE_CLASSIFICATIONS,
    listLabel: 'List of Excise Classifications',
  },
  { key: 'tcs', label: 'TCS', list: TCS_NATURE_OF_GOODS, listLabel: 'List of Nature of Goods' },
];
// F11 tax features — hide a tax type from the class config when its flag is off (GST not gated).
const TAX_TYPE_FEATURE: Partial<Record<TaxType, TaxFeature>> = {
...
const cell =
  'border border-zinc-300 px-1.5 py-1 text-xs outline-none focus:border-zinc-800 bg-white w-full';
const th =
  'border border-zinc-300 px-1.5 py-1 text-[10px] font-bold uppercase tracking-wide text-zinc-600 bg-zinc-50 text-left';
...
export default function VoucherClassConfigPopup({
  ...
  const visibleTaxTypes = TAX_TYPE_META.filter((m) => {
    const f = TAX_TYPE_FEATURE[m.key];
  ...
  const ledgerName = (id?: number) =>
    id ? (ledgers.find((l) => l.ledger_id === id)?.name ?? `#${id}`) : '';
  ...
  const patchAlloc = (id: string, p: Partial<ClassAllocationRow>) =>
    setForm((f) => ({
      ...f,
      default_allocations: (f.default_allocations ?? []).map((r) =>
        r.id === id ? { ...r, ...p } : r,
      ),
    }));
  const patchAddl = (id: string, p: Partial<ClassAdditionalEntryRow>) =>
    setForm((f) => ({
      ...f,
      additional_entries: (f.additional_entries ?? []).map((r) =>
        r.id === id ? { ...r, ...p } : r,
      ),
    }));
  const addAllocRow = () =>
    setForm((f) => ({
      ...f,
      default_allocations: [...(f.default_allocations ?? []), { id: newId('alloc') }],
    }));
  const addAddlRow = () =>
    setForm((f) => ({
      ...f,
      additional_entries: [...(f.additional_entries ?? []), { id: newId('addl') }],
    }));
  const removeAlloc = (id: string) =>
    setForm((f) => ({
      ...f,
      default_allocations: (f.default_allocations ?? []).filter((r) => r.id !== id),
    }));
  const removeAddl = (id: string) =>
    setForm((f) => ({
      ...f,
      additional_entries: (f.additional_entries ?? []).filter((r) => r.id !== id),
    }));
  ...
  const pickerItems: SelectionItem[] = useMemo(() => {
    ...
    if (picker.kind === 'exclude' || picker.kind === 'include') {
      ...
      return groups
        .filter(
          (g) => g.group_id != null && !taken.has(g.group_id) && g.name.toLowerCase().includes(q),
        )
  ...
  const acceptPick = (i: number) => {
    ...
    if (picker.kind === 'exclude' || picker.kind === 'include') {
      ...
      patchAlloc(picker.rowId, {
        tax_classifications: { ...(row?.tax_classifications ?? {}), [picker.taxType]: item.name },
      });
          ...
          {ids.length === 0 ? (
            ...
            ids.map((id) => (
              <div key={id} className="flex items-center justify-between px-2 py-1 text-xs">
                ...
                <button
                  onClick={() => removeGroup(side, id)}
                  className="text-zinc-300 hover:text-zinc-900 font-bold px-1"
                >
                  &times;
                </button>
              </div>
    ...
    <div className="fixed inset-0 z-40 flex flex-col bg-white select-none">
        ...
        <div className="max-w-[1100px] mx-auto space-y-6 py-2">
          ...
          <section>
            <p className="text-[11px] font-semibold text-zinc-700 mb-1">
              Default Accounting Allocations for all items in Invoice (except for the items
              specified below)
            </p>
                ...
                {allocs.map((r) => (
                  <tr key={r.id}>
                    <td className="p-0 w-[24%]">
                      <LedgerCell table="alloc" rowId={r.id} value={ledgerName(r.ledger_id)} />
                    </td>
                    <td className="p-0">
                      <select
                        className={cell}
                    ...
                    <td className="p-0 w-[12%] relative">
                      <input
                        className={`${cell} text-right pr-5`}
                        type="number"
                        value={r.percentage ?? ''}
                        onChange={(e) =>
                          patchAlloc(r.id, {
                      ...
                      />
                      <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-xs text-zinc-500 pointer-events-none">
                        %
                      </span>
                    </td>
                    <td className="p-0">
                      <input
                        className={cell}
                        value={r.rounding_method ?? ''}
                        onChange={(e) => patchAlloc(r.id, { rounding_method: e.target.value })}
                    ...
                    <td className="p-0 w-[14%]">
                      <select
                        className={cell}
                        value={r.override_item_default ?? 'No'}
                        onChange={(e) =>
                          patchAlloc(r.id, {
                            override_item_default: e.target.value as 'Yes' | 'No',
                          })
                        }
                      >
                        <option>No</option>
                    ...
                    <td className="p-0 w-8 text-center">
                      <button
                        onClick={() => removeAlloc(r.id)}
                        className="text-zinc-300 hover:text-zinc-900 font-bold px-2"
                      >
                        &times;
                      </button>
                    </td>
            ...
            <button
              onClick={addAllocRow}
              className="mt-1 text-[10px] font-bold uppercase tracking-wide text-zinc-500 hover:text-zinc-900"
            >
              + Add allocation
            </button>
          </section>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {addl.map((r) => (
                  <tr key={r.id}>
                    <td className="p-0 w-[24%]">
                      <LedgerCell table="addl" rowId={r.id} value={ledgerName(r.ledger_id)} />
                    </td>
                    <td className="p-0">
                      <input
                        className={cell}
                        value={r.type_of_calculation ?? ''}
                        onChange={(e) => patchAddl(r.id, { type_of_calculation: e.target.value })}
                      />
                    </td>
                    <td className="p-0 w-[12%]">
                      <input
                        ...
                        onChange={(e) =>
                          patchAddl(r.id, {
                            value_basis: e.target.value === '' ? undefined : Number(e.target.value),
                          })
                        }
                      />
                    </td>
                    <td className="p-0">
                      <input
                        className={cell}
                        value={r.rounding_method ?? ''}
                        onChange={(e) => patchAddl(r.id, { rounding_method: e.target.value })}
                        placeholder="Not Applicable"
                      />
                    </td>
                    <td className="p-0 w-[10%]">
                    ...
                    <td className="p-0 w-[12%]">
                      <select
                        className={cell}
                        value={r.remove_if_zero ?? 'No'}
                        onChange={(e) =>
                          patchAddl(r.id, { remove_if_zero: e.target.value as 'Yes' | 'No' })
                        }
                      >
                        <option>No</option>
                        <option>Yes</option>
      ...
      {taxClassRowId &&
        (() => {
          ...
          return (
            <div
              className="fixed inset-0 z-40 flex items-start justify-center bg-zinc-900/30 pt-24 select-none"
              role="dialog"
            >
              <div className="bg-white border border-zinc-400 shadow-xl w-[420px]">
                ...
                {visibleTaxTypes.map((m) => (
                  <div
                    key={m.key}
                    className="flex items-center px-3 py-1.5 border-b border-zinc-100"
                  >
                    <span className="w-32 text-xs font-bold text-zinc-800">{m.label}</span>
                    <button
                      onClick={() =>
                        openPicker({ kind: 'classification', taxType: m.key, rowId: row.id })
                      }
                ...
                <div className="flex justify-end gap-2 px-3 py-2 bg-zinc-50 border-t border-zinc-300">
                  <button
                    onClick={() => setTaxClassRowId(null)}
                    className="text-xs px-4 py-1 bg-zinc-950 text-white hover:bg-zinc-800 font-semibold"
                  >
                    Accept
                  </button>
                </div>
      ...
      {picker && (
        <SelectionPopup
          title={
            picker.kind === 'ledger'
              ? 'Select Ledger'
              : picker.kind === 'classification'
                ? 'Select Classification'
                : picker.kind === 'exclude'
                  ? 'Select Group (Exclude)'
          ...
          listLabel={
            picker.kind === 'ledger'
              ? 'List of Ledgers'
              : picker.kind === 'classification'
                ? (TAX_TYPE_META.find((m) => m.key === picker.taxType)?.listLabel ?? 'List')
                : 'List of Groups'
          }
          items={pickerItems}
          index={pickIdx}
