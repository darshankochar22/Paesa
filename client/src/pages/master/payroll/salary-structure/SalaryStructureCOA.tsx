  ...
  const filtered = useMemo(() => {
    ...
    if (q)
      result = result.filter(
        (s) =>
          empName(s.employee_id).toLowerCase().includes(q) ||
          payHeadName(s.pay_head_id).toLowerCase().includes(q),
      );
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.altKey && e.key.toLowerCase() === 'c') {
        e.preventDefault();
        navigate('/master/create/salary-structure');
      }
    };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-800">
      <div className="px-4 py-2 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
        ...
        <button
          onClick={() => navigate('/master/create/salary-structure')}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="px-4 py-1.5 border-b border-zinc-100 flex items-center gap-2">
            ...
            <input
              ...
              placeholder="Search by employee, pay head..."
            />
            {searchQuery && (
              <button
                ...
                return (
                  <div key={sId}>
                    <div
                      ...
                      onClick={() =>
                        navigate('/master/alter/salary-structure', { state: { structureId: sId } })
                      }
                    >
