...
export default function EmployeeGroupCOA() {
  ...
  const buildTree = (parentId: number | null): TreeNode[] => {
    ...
    if (q) {
      items = items.filter(
        (g) => g.name.toLowerCase().includes(q) || g.alias?.toLowerCase().includes(q),
      );
    }
    if (showUnusedOnly) {
      items = items.filter((g) =>
        employees.every((e) => e.employee_group_id !== g.employee_group_id),
      );
    }
    return items.map((g) => ({
      ...g,
  ...
  const renderTree = (nodes: TreeNode[], depth: number = 0): React.ReactNode => {
    return nodes.map((node) => {
      ...
      const empCount = employees.filter(
        (e) => e.employee_group_id === node.employee_group_id,
      ).length;
      ...
      return (
        <div key={nodeId}>
          <div
            ...
            onClick={() =>
              navigate('/master/alter/employee-group', {
                state: { groupId: node.employee_group_id },
              })
            }
          >
            <span
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.altKey && e.key.toLowerCase() === 'c') {
        e.preventDefault();
        navigate('/master/create/employee-group');
      }
    };
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              ...
              tree.forEach((n) => {
                allIds[`g-${n.employee_group_id}`] = true;
              });
          ...
          <button
            onClick={() => navigate('/master/create/employee-group')}
            className="text-[10px] text-zinc-500 hover:text-zinc-800 border border-zinc-200 rounded px-2 py-0.5 bg-white"
          >
            + Create
          </button>
