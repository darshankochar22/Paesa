    ...
    if (q)
      result = result.filter(
        (e) =>
          ...
          groupName(e.employee_group_id).toLowerCase().includes(q),
      );
    ...
    return nodes.map((group) => {
      ...
      return (
        <div key={nodeId}>
          ...
          >
            <span
              className="text-zinc-400 text-xs mr-1 w-4 text-center"
              onClick={(e) => {
                e.stopPropagation();
                ...
                return (
                  <div key={`e-${eId}`}>
                    <div
                      ...
                      onClick={() =>
                        navigate('/master/alter/employee', { state: { employeeId: eId } })
                      }
                    >
                      <span className="text-zinc-300 text-xs mr-2">▫</span>
                      <span className="flex-1 text-sm text-zinc-700 group-hover:text-sky-800 transition-colors">
                        {emp.name}
                      </span>
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-800">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              ...
              groups.forEach((g) => {
                ids[`g-${g.employee_group_id}`] = true;
              });
          ...
          <button
            onClick={() => navigate('/master/create/employee')}
            className="text-[10px] text-zinc-500 hover:text-zinc-800 border border-zinc-200 rounded px-2 py-0.5 bg-white"
          >
            + Create
          </button>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="px-4 py-1.5 border-b border-zinc-100 flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-300 ml-2">
              | F5: {viewMode === 'tree' ? 'Flat' : 'Tree'} View
            </span>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              buildTree(null).length === 0 ? (
                ...
                renderGroupTree(buildTree(null))
              )
            ) : flatList.length === 0 ? (
              <div className="p-8 text-center text-xs text-zinc-400">No employees found.</div>
            ) : (
              <>
                ...
                {flatList.map((emp) => {
                  ...
                  return (
                    <div key={eId}>
                      <div
                        className="group grid grid-cols-12 gap-1 px-4 py-1 border-b border-zinc-50 hover:bg-zinc-50/50 cursor-pointer text-xs"
                        onClick={() =>
                          navigate('/master/alter/employee', { state: { employeeId: eId } })
                        }
                      >
                        <span className="col-span-3 font-medium text-zinc-700 truncate group-hover:text-sky-800 transition-colors">
                          {emp.name}
                        </span>
                        <span className="col-span-2 text-zinc-500">{emp.employee_code || '-'}</span>
                        <span className="col-span-3 text-zinc-500">
                          {groupName(emp.employee_group_id)}
                        </span>
      ...
      <div className="border-t border-zinc-200 px-4 py-1.5 flex justify-between items-center bg-zinc-50 text-[10px] text-zinc-400">
        <span>
          {filteredEmployees.length} employee(s) | {viewMode === 'tree' ? 'Tree View' : 'Flat View'}
        </span>
