    ...
    if (q)
      result = result.filter(
        (p) =>
          ...
          p.pay_head_type?.toLowerCase().includes(q) ||
          p.calculation_type?.toLowerCase().includes(q),
      );
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.altKey && e.key.toLowerCase() === 'c') {
        e.preventDefault();
        navigate('/master/create/pay-head');
      }
    };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-800">
      <div className="px-4 py-2 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
        ...
        <button
          onClick={() => navigate('/master/create/pay-head')}
                ...
                return (
                  <div key={pId}>
                    <div
                      ...
                      onClick={() =>
                        navigate('/master/alter/pay-head', { state: { payHeadId: pId } })
                      }
                    >
