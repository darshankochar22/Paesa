      ...
      return (
        <div key={gId} className="flex flex-col">
          ...
          {isExpanded && hasChildren && (
            <div className="flex flex-col">{renderGodownTree(node.children!, depth + 1)}</div>
          )}
        </div>
      );
    });
      ...
      <div className="px-4 py-2 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-sm text-zinc-800">
            Godowns & Locations Chart of Accounts
          </span>
          {showUnusedOnly && (
