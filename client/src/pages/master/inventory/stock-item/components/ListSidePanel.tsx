...
export default function ListSidePanel({
  ...
  return (
    <div
      className="w-64 border-l border-zinc-300 flex flex-col bg-white shrink-0 font-mono shadow-md select-none"
      data-enter-nav-ignore
    >
      <div className="bg-white text-zinc-700 text-xs px-3 py-1.5 font-bold uppercase tracking-wider border-b border-zinc-200">
        {title}
      </div>
      <input
        ref={inputRef}
      ...
      <div className="border-t border-zinc-200 px-3 py-1.5 bg-zinc-50">
        <button onClick={onClose} className="text-xs text-zinc-500 hover:text-zinc-800 font-sans">
          Esc: Close
        </button>
      </div>
    </div>
