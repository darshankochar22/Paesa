...
export default function ExciseTariffDetails({
  ...
  const [activeDropdown, setActiveDropdown] = useState<'reporting_uom' | 'valuation_type' | null>(
    null,
  );
  ...
  const showRate =
    form.valuation_type === 'Ad Valorem' || form.valuation_type === 'Valorem + Quantum';
  const showRatePerUnit =
    form.valuation_type === 'Ad Quantum' || form.valuation_type === 'Valorem + Quantum';
    ...
    >
      <div className="bg-white border border-zinc-400 w-[520px] flex flex-col shadow-xl">
        ...
        <div className="px-6 py-4 space-y-2">
          ...
          >
            <span className="w-48 shrink-0 text-sm text-zinc-700 font-sans">
              Reporting unit of measure
            </span>
            <span className="w-4 shrink-0 text-zinc-400 text-sm text-center">:</span>
            <div className="flex-1">
              <span className="text-sm text-zinc-955 font-bold">
                {EXCISE_REPORTING_UOM_OPTIONS.find((o) => o.id === form.reporting_uom)?.label ||
                  form.reporting_uom}
              </span>
            </div>
          </div>
          ...
          >
            ...
            <div className="flex-1">
              <span className="text-sm text-zinc-955 font-bold">
                {EXCISE_VALUATION_TYPE_OPTIONS.find((o) => o.id === form.valuation_type)?.label ||
                  form.valuation_type}
              </span>
            </div>
          </div>
        ...
        {activeDropdown === 'reporting_uom' && (
          <div className="absolute right-0 top-0 w-72 border-l border-zinc-300 flex flex-col bg-white shadow-md font-mono max-h-full">
            <div className="bg-zinc-800 text-white text-xs px-3 py-1.5 font-bold uppercase tracking-wider shrink-0">
              List of Excise Reporting UoMs
            </div>
            <div className="overflow-y-auto min-h-0 py-0.5 flex-1">
              {EXCISE_REPORTING_UOM_OPTIONS.map((opt) => (
                <div
                  ...
                  onClick={() => {
                    setForm((f) => ({ ...f, reporting_uom: opt.id }));
            ...
            <div className="overflow-y-auto min-h-0 py-0.5 flex-1">
              {EXCISE_VALUATION_TYPE_OPTIONS.map((opt) => (
                <div
                  ...
                  onClick={() => {
                    setForm((f) => ({ ...f, valuation_type: opt.id }));
                    setActiveDropdown(null);
                  }}
                >
                  <span className="truncate">{opt.label}</span>
