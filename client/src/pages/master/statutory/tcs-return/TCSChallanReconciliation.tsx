...
// TCS Challan Reconciliation (#203) — mirrors the TDS screen (#199); a TCS challan is a
// Payment voucher on a type_of_duty_tax='TCS' ledger, and the collectee/portal columns
// stay blank until reconciled with the filed challan.
export default function TCSChallanReconciliation() {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await window.api.tcs.getChallanReconciliation({
      ...
      if (res.success) {
        ...
        setError(res.error || 'Failed to load TCS Challan reconciliation.');
      }
    } catch (e: any) {
  ...
  return (
    ...
    >
      <div className="w-full flex flex-col font-sans text-xs pb-4">
        {loading && <EmptyState message="Loading TCS challan reconciliation…" className="italic" />}
                ...
                {challans.length === 0 ? (
                  <TableRow className="hover:bg-transparent">
                    <TableCell colSpan={14} className="text-center py-4 text-gray-400 italic">
                      No TCS challan payments found for this Financial Year.
                    </TableCell>
                  </TableRow>
                ) : (
                  challans.map((c, idx) => (
                    ...
                    >
                      ...
                      <TableCell className={CELL}>{c.collectee_type}</TableCell>
