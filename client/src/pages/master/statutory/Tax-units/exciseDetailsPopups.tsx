...
function ExciseTariffPopup({
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-[60] font-mono text-[11px]">
      <div className="bg-white border border-zinc-800 shadow-2xl w-[520px] p-5" data-enter-nav>
        ...
        <div className="py-4 space-y-2">
          ...
          <FormRow label="HSN code" labelWidth="w-52">
            <input
              className={popupInput}
              value={tariff.hsn}
              onChange={(e) => upd({ hsn: e.target.value })}
            />
          </FormRow>
          <FormRow label="Reporting unit of measure" labelWidth="w-52">
            <select
              className={popupInput}
              value={tariff.uom}
              onChange={(e) => upd({ uom: e.target.value })}
            >
          ...
          <FormRow label="Valuation type" labelWidth="w-52">
            <select
              ...
              onChange={(e) => upd({ valuationType: e.target.value })}
            >
            ...
            <FormRow label="Rate" labelWidth="w-52">
              <span className="flex items-center gap-1">
                <input
                  ...
                  min={0}
                  step="0.01"
                  className="w-44 bg-zinc-50 border border-zinc-300 px-2 py-0.5 outline-none focus:border-zinc-800 font-mono font-bold text-xs text-right"
                  value={tariff.rate}
          ...
          {showsRatePerUnit(tariff.valuationType) && (
            <FormRow label="Rate per Unit" labelWidth="w-52">
              <input
                ...
                step="0.01"
                className="w-44 bg-zinc-50 border border-zinc-300 px-2 py-0.5 outline-none focus:border-zinc-800 font-mono font-bold text-xs text-right"
                value={tariff.ratePerUnit}
                onChange={(e) => upd({ ratePerUnit: e.target.value })}
              />
            </FormRow>
          )}
        </div>
        <div className="border-t border-zinc-200 pt-3 flex justify-end">
          <button
            data-enter-accept
            onClick={onClose}
            className="text-[11px] px-4 py-1 border border-zinc-800 bg-zinc-900 text-white hover:bg-black font-bold"
          >
            Ok
          </button>
        </div>
      ...
      <div className="bg-white border border-zinc-800 shadow-2xl w-[320px] max-h-[70vh] flex flex-col">
        ...
        <div className="border-t border-zinc-200 flex">
          <button
            ...
            className="flex-1 py-2 text-center hover:bg-zinc-100 border-r border-zinc-200 font-bold"
          >
            Create
          </button>
          <button onClick={onClose} className="flex-1 py-2 text-center hover:bg-zinc-100">
            Cancel
          </button>
        </div>
      </div>
...
export function ExciseDetailsPopup({
  ...
  const toggleTariff = (v: boolean) => {
    setSetAlterTariff(v);
    if (v) setShowTariff(true);
  };
  const toggleRule11 = (v: boolean) => {
    setSetAlterRule11(v);
    if (v) setShowBook(true);
  };

  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 font-mono text-[11px]">
      <div className="bg-white border border-zinc-800 shadow-2xl w-[580px] p-5" data-enter-nav>
        <div className="text-center font-bold text-xs pb-3 border-b border-zinc-200 uppercase tracking-wide">
          Excise Details
        ...
        <div className="py-4 space-y-2">
          ...
          {isManufacturer && (
            <FormRow label="Type of manufacturer" labelWidth="w-56">
              <select
                className={popupInput}
                value={typeOfManufacturer}
                onChange={(e) => setTypeOfManufacturer(e.target.value)}
              >
          ...
          <FormRow label="Set/alter excise tariff details" labelWidth="w-56">
            <select
              ...
              onChange={(e) => toggleTariff(e.target.value === 'Yes')}
            >
          ...
          {setAlterTariff && tariff.name && (
            <div className="pl-[15rem] text-[10px] text-zinc-500 italic">
              {tariff.name} · {tariff.valuationType}
            </div>
          )}
          ...
          <FormRow label="Set/alter Rule 11 book details" labelWidth="w-56">
            <select
              ...
              onChange={(e) => toggleRule11(e.target.value === 'Yes')}
            >
              <option value="No">No</option>
              <option value="Yes">Yes</option>
            </select>
