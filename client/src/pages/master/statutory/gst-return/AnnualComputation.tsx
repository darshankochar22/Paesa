...
export default function AnnualComputation() {
  ...
  const openStats = () =>
    navigate('/master/statutory/gst/return-statistics', { state: annualState });
  const openNotRelevant = () =>
    navigate('/master/statutory/gst/not-relevant', { state: annualState });
  // Uncertain drills to the correction-reason breakdown tree (Annual = inward + outward).
  const openUncertain = () =>
    navigate('/master/statutory/annual-computation/uncertain', {
      state: {
        registration,
        returnType: 'ANNUAL',
        reportTitle: 'Annual Computation',
  ...
  const fyLabel =
    data?.fy_label ?? (activeFY ? `${activeFY.start_date} to ${activeFY.end_date}` : '');
  ...
  const rows: RowDef[] = [
    ...
    {
      type: 'data',
      label: 'Missing Invoice Reported in Current Period',
      data: liab_missing,
      indent: 1,
      nav: () =>
        openRegister('Missing Invoice Reported in Current Period', {
          bucket: 'included',
          voucherType: '__none__',
        }),
    },
    ...
    {
      type: 'total',
      label: 'Total ITC After Reversal & Ineligible Input Tax Credit',
      data: itc_net,
    },
    ...
    {
      type: 'data',
      label: 'Summary of Outward Supplies',
      data: outward_summ,
      indent: 1,
      nav: () =>
        openSection('HSN-SAC Summary (Outward Supplies)', { section: 'hsn', direction: 'outward' }),
    },
    {
      type: 'data',
      label: 'Summary of Inward Supplies',
      data: inward_summ,
      indent: 1,
      nav: () =>
        openSection('HSN-SAC Summary (Inward Supplies)', { section: 'hsn', direction: 'inward' }),
    },
  ];
            ...
            <Table className="text-xs table-fixed">
              ...
              <TableFooter className="bg-transparent">
                <TableRow className="border-t border-gray-400 hover:bg-transparent font-bold">
                  ...
                  <TableCell className={cn(NUM_CELL, 'font-bold')}>
                    {fmt(grandTotal.txval)}
                  </TableCell>
                  <TableCell className={cn(NUM_CELL, 'font-bold')}>
                    {fmt(grandTotal.iamt)}
                  </TableCell>
