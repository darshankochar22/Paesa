...
export default function GSTVoucherRegister() {
  ...
  const columns: "accounting" | "tax" =
    location.state?.columns || (bucket === "not_relevant" || bucket === "all" ? "accounting" : "tax");
  ...
  useEffect(() => {
    ...
    load();
  }, [companyId, fyId, month, year, returnType, bucket, category, voucherType, registration?.gst_id, annual, location.state?.section, location.state?.direction, location.state?.annualCategory]);
          ...
          <Table className="text-xs table-fixed">
            <TableHeader>
              <TableRow className="border-b border-gray-300 hover:bg-transparent">
                <TableHead className="h-auto w-24 px-2 py-1 align-bottom font-bold text-black">Date</TableHead>
                <TableHead className="h-auto px-2 py-1 align-bottom font-bold text-black">Particulars</TableHead>
                <TableHead className="h-auto w-28 px-2 py-1 align-bottom font-bold text-black">Vch Type</TableHead>
                <TableHead className="h-auto w-16 px-2 py-1 text-center align-bottom font-bold text-black">Vch No.</TableHead>
                <TableHead className="h-auto w-32 px-2 py-1 text-right align-bottom font-bold text-black">Debit<br />Amount</TableHead>
                <TableHead className="h-auto w-32 px-2 py-1 text-right align-bottom font-bold text-black">Credit<br />Amount</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {rows.length === 0 ? (
