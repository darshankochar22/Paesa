...
interface TCSNatureOfGoodsSelectionPanelProps {
  tcsList: TCSNatureOfGoodsType[];
  onSelect: (t: TCSNatureOfGoodsType) => void;
  onCancel: () => void;
  onCreate: () => void;
}

export default function TCSNatureOfGoodsSelectionPanel({
  tcsList,
  onSelect,
  onCancel,
  onCreate,
}: TCSNatureOfGoodsSelectionPanelProps) {
  ...
  const filtered = tcsList.filter((t) =>
  ...
  const selectionActions = [
    { key: "Alt+C", label: "Create TCS Nature", onClick: onCreate },
    { key: "Esc", label: "Quit", onClick: onCancel },
  ];

  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none font-sans">
      <PageTitleBar title="Alter TCS Nature of Goods" subtitle="Select TCS Nature of Goods to Alter" />
      <div className="p-3 bg-zinc-50 border-b border-zinc-200 shrink-0">
        <SearchInput
          value={search}
          onChange={setSearch}
          placeholder="Search nature of goods by name, section or payment code…"
          autoFocus
        />
      </div>
      <div className="flex-1 flex min-h-0">
        <div className="flex-1 flex flex-col bg-white border-r border-zinc-100">
          <DataTable
            columns={columns}
            rows={filtered}
            rowKey={(r: TCSNatureOfGoodsType) => String(r.tcs_id)}
            onRowClick={onSelect}
            emptyMessage="No TCS Nature of Goods records found."
          />
        </div>
