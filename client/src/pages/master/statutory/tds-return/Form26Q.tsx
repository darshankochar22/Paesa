...
export default function Form26Q() {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await window.api.tds.getForm26Q({ company_id: companyId, fy_id: fyId });
      if (res.success) setData(res.payload);
      else setError(res.error || 'Failed to load Form 26Q.');
    } catch (e: any) {
  ...
  return (
    ...
    >
      <div className="w-full flex flex-col font-sans text-xs pb-4">
        {loading && <EmptyState message="Computing Form 26Q…" className="italic" />}
