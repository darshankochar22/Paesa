...
export default function LedgerMonthlySummaryLayout() {
  ...
  React.useEffect(() => {
    if (!ledgerId || !selectedCompany?.company_id || !activeFY?.fy_id) {
      setLoading(false);
      return;
    }
    setLoading(true);
    ...
    <div className="flex flex-col h-full w-full overflow-hidden bg-white font-mono">
      ...
      <div className="bg-white border-b border-gray-200 px-3 py-2 flex justify-between items-start">
        ...
        <div className="text-right">
          <div className="text-[11px] italic text-black">{data.ledger_name}</div>
        ...
        <table className="w-full border-collapse font-mono text-[11px]">
          <tbody>
            <tr className="border-b border-gray-200">
              <td className="px-3 py-1 italic text-black">Opening Balance</td>
      ...
      <div className="border-t border-gray-200 px-4 py-3" style={{ height: chartHeight + 30 }}>
        <svg
          width="100%"
          height={chartHeight}
          viewBox={`0 0 ${data.rows.length * 60} ${chartHeight}`}
        >
          {data.rows.map((row, i) => {
