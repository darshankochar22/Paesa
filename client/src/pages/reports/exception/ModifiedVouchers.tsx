...
export default function ModifiedVouchers() {
  ...
  const [rows, setRows] = React.useState<ModifiedRow[]>([]);
  ...
  React.useEffect(() => {
    ...
    (window as any).api.report
      ...
      .then((res: any) => {
        ...
        else setError(res?.error || 'Failed to load modified vouchers');
        setLoading(false);
      })
      .catch((e: any) => {
        setError(e?.message || 'Failed to load modified vouchers');
        setLoading(false);
      });
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-black font-sans text-[11px]">
      <div className="flex items-center justify-between px-3 py-1.5 bg-white border-b-2 border-gray-200">
        <span className="font-bold text-sm tracking-wide">Modified Vouchers Register</span>
      ...
      <div className="flex justify-between items-center px-3 py-1.5 bg-white border-b border-gray-200 font-mono">
        <span>Modified Vouchers Register</span>
          ...
          <thead className="sticky top-0 bg-white border-b border-gray-200 z-10 text-black">
            <tr>
              ...
              <th className="px-3 py-1 text-right font-bold w-32 border-l border-gray-200">
                Debit Amount
                <br />
                <span className="font-normal text-black">Inwards Qty</span>
              </th>
              <th className="px-3 py-1 text-right font-bold w-32 border-l border-gray-200">
                Credit Amount
                <br />
                <span className="font-normal text-black">Outwards Qty</span>
              </th>
            </tr>
          </thead>
          <tbody>
              ...
              rows.map((row, idx) => {
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-200 cursor-pointer ${isFocused ? 'bg-black/[0.06] text-black font-bold' : 'hover:bg-black/[0.03] text-black'} ${row.is_optional ? 'italic' : ''}`}
                  >
                    <td className="px-3 py-1 whitespace-nowrap">{formatDate(row.date)}</td>
                    <td className="px-3 py-1 truncate max-w-xs">
                      {row.particulars || '—'}
                      {row.is_optional ? <span className="ml-1 text-black">(Optional)</span> : null}
                    </td>
      ...
      <div className="border-t-2 border-black bg-white px-3 py-1.5 flex font-mono text-[11px] font-bold text-black shrink-0">
        ...
        <span className="w-32" />
        <span className="w-20" />
        <span className="w-32 text-right pr-2 border-l border-gray-200">{fmtAmt(totalDebit)}</span>
        <span className="w-32 text-right pr-2 border-l border-gray-200">{fmtAmt(totalCredit)}</span>
      </div>
    </div>
  );
}
