...
export default function ReorderStatus() {
  ...
  const openSelect = React.useCallback(
    (scope: Scope) => {
      ...
      const p =
        scope === 'category'
          ? api.stockCategory.getAll(companyId)
          : api.stockGroup.getAll(companyId);
      p.then((res: any) => {
  ...
  const filtered = React.useMemo(
    () =>
      search.trim() === ''
        ? masters
        : masters.filter((m) => m.name.toLowerCase().includes(search.toLowerCase())),
    [masters, search],
  );
  React.useEffect(() => {
    setSelectIdx(0);
  ...
  const loadReport = React.useCallback(
    (scope: Scope, ref: ScopeRef) => {
      ...
      (window as any).api.report
        .reorderStatusScoped(companyId, fyId, scope, ref.id)
        .then((res: any) => {
  ...
  const loadMovement = React.useCallback(
    (scope: Scope, ref: ScopeRef, item: ReorderItem) => {
      if (!companyId || !fyId) return;
  ...
  const voucherRows = React.useMemo(() => {
    ...
    return allRowsRef.current.filter(
      (r) => r.voucher_id && r.particulars === level.party && rowFamily(r) === level.direction,
    );
  }, [level]);
  ...
  const openVouchers = React.useCallback((section: MovSection, party: string) => {
    setLevel((l) =>
      l.step === 'movement'
        ? { step: 'vouchers', scope: l.scope, ref: l.ref, item: l.item, party, direction: section }
        : l,
    );
    setVoucherIdx(0);
  }, []);
  ...
  React.useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (level.step === 'submenu') {
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          setSubIdx((p) => Math.min(SUB.length - 1, p + 1));
        } else if (e.key === 'ArrowUp') {
          e.preventDefault();
          setSubIdx((p) => Math.max(0, p - 1));
        } else if (e.key === 'Enter') {
          e.preventDefault();
          openSelect(SUB[subIdx].scope);
        ...
        if (e.key === 'ArrowDown') {
          ...
          e.preventDefault();
          const m = filtered[selectIdx];
          if (m) loadReport(level.scope, m);
        } else if (e.key === 'Escape' || e.key === 'Backspace') {
          e.preventDefault();
          setLevel({ step: 'submenu' });
        }
        ...
        if (e.key === 'ArrowDown') {
          ...
          if (it) loadMovement(level.scope, level.ref, it);
        } else if (e.key === 'Escape' || e.key === 'Backspace') {
          e.preventDefault();
          openSelect(level.scope);
        }
      } else if (level.step === 'movement') {
        const inLen = movData?.inward.length ?? 0,
          outLen = movData?.outward.length ?? 0;
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          setMovCursor((c) =>
            c.section === 'inward'
          ...
          e.preventDefault();
          loadReport(level.scope, level.ref);
        }
      } else {
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          ...
          e.preventDefault();
          loadMovement(level.scope, level.ref, level.item);
        }
      }
    };
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <SelectionPopup
          ...
          onCreate={() =>
            navigate(isCat ? '/master/create/stock-category' : '/master/create/stock-group')
          }
        />
      </div>
    );
