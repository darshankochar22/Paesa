...
export default function AgeingAnalysis() {
  ...
  const loadReport = React.useCallback(
    (group: GroupRef) => {
      ...
      (window as any).api.report
        .stockAgeingAnalysis(companyId, fyId, group.group_id, asAt, fyStart)
        .then((res: any) => {
          if (res.success) {
            setRows(res.rows ?? []);
  ...
  const loadMonthly = React.useCallback(
    (group: GroupRef, item: RawRow) => {
  ...
  const loadVouchers = React.useCallback(
    (group: GroupRef, item: RawRow, mIdx: number) => {
      if (!companyId || !fyId) return;
  ...
  const backToMonthly = React.useCallback((group: GroupRef, item: RawRow) => {
    setLevel({ step: 'monthly', group, item });
    setVouchers([]);
  }, []);
    ...
    const h = (e: KeyboardEvent) => {
      if (level.step === 'select') {
        ...
        if (e.key === 'ArrowDown') {
          ...
          e.preventDefault();
          const it = rows[rowIdx];
          if (it) loadMonthly(level.group, it);
        ...
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          setMonthIdx((p) => Math.min(months.length - 1, p + 1));
        } else if (e.key === 'ArrowUp') {
          e.preventDefault();
          setMonthIdx((p) => Math.max(-1, p - 1));
        } else if (e.key === 'Enter') {
          e.preventDefault();
          if (monthIdx >= 0) loadVouchers(level.group, level.item, monthIdx);
        } else if (e.key === 'Escape' || e.key === 'Backspace') {
          e.preventDefault();
          loadReport(level.group);
        }
      } else {
        if (e.key === 'ArrowDown') {
          e.preventDefault();
  ...
  if (level.step === 'report') {
    ...
    return (
      <StockAgeingTable
        companyName={selectedCompany?.name}
        groupLabel={level.group.group_name}
        asAt={reportAsAt}
        bands={bands}
        rows={ageRows}
        ...
        footer={
          <FooterBar>
            ...
            <span className="text-black">Enter: Stock item monthly summary</span>
          </FooterBar>
        }
      />
    );
  ...
  if (level.step === 'monthly') {
    return (
      <StockItemMonthlyTable
        ...
        onSelectIndex={setMonthIdx}
        onActivate={(i) => loadVouchers(level.group, level.item, i)}
        footer={
          <FooterBar>
            <button
              onClick={() => loadReport(level.group)}
              className="hover:underline hover:text-black"
            >
              Q: Back to Ageing
            </button>
            <span className="text-black">Enter: Stock item vouchers</span>
          </FooterBar>
        }
      />
    );
  ...
  return (
    <StockItemVouchersTable
      ...
      footer={
        <FooterBar>
          <button
            onClick={() => backToMonthly(group, item)}
            className="hover:underline hover:text-black"
          >
            Q: Back to Monthly Summary
          </button>
