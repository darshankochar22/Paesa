...
export default function StockCategorySummary() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.stockCategory
      .getAll(companyId)
      .then((res: any) => {
        if (res.success) {
          const list = [...(res.stockCategories ?? [])].sort((a, b) =>
  ...
  const loadMonths = React.useCallback(
    (category: CategoryRow, item: ItemRow) => {
      if (!companyId || !fyId) return;
  ...
  const loadVouchers = React.useCallback(
    (category: CategoryRow, item: ItemRow, fromDate?: string, toDate?: string) => {
      if (!companyId || !fyId) return;
  ...
  const backToCategories = React.useCallback(() => {
    setLevel({ step: 'category' });
    setItems([]);
  }, []);
  const backToSummary = React.useCallback((category: CategoryRow) => {
    setLevel({ step: 'summary', category });
    setMonths([]);
  }, []);
  const backToMonthly = React.useCallback((category: CategoryRow, item: ItemRow) => {
    setLevel({ step: 'monthly', category, item });
    setVoucherRows([]);
  }, []);
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setCategoryIndex((p) => Math.min(categories.length - 1, p + 1));
        return;
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault();
        setCategoryIndex((p) => Math.max(0, p - 1));
        return;
      }
      if (e.key === 'Enter') {
        e.preventDefault();
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        ...
        if (it) loadMonths(level.category, it);
        return;
      }
      if (e.key === 'Escape' || e.key === 'Backspace') {
        e.preventDefault();
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape' || e.key === 'Backspace') {
        e.preventDefault();
        backToMonthly(level.category, level.item);
      }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  ...
  if (level.step === 'category') {
    return (
      <SelectionPopup
        ...
        items={categories.map((c) => ({ id: c.sc_id, name: c.name }))}
        index={categoryIndex}
        loading={loadingCategories}
        emptyText="No stock categories found."
