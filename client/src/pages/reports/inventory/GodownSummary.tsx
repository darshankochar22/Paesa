...
export default function GodownSummary() {
  ...
  const summaryEntries = React.useMemo<SummaryEntry[]>(() => {
    const groups = new Map<
      number,
      { name: string; qty: number; value: number; units: Set<string> }
    >();
    const entries: SummaryEntry[] = [];
    for (const it of items) {
      if (it.group_id != null) {
        if (!groups.has(it.group_id)) {
          groups.set(it.group_id, {
            name: it.group_name || 'Ungrouped',
            qty: 0,
            value: 0,
            units: new Set(),
          });
  ...
  const loadMonths = React.useCallback(
    (godown: GodownRow, item: ItemRow, group?: GroupMeta) => {
      if (!companyId || !fyId) return;
  ...
  const loadVouchers = React.useCallback(
    (godown: GodownRow, item: ItemRow, fromDate?: string, toDate?: string, group?: GroupMeta) => {
      if (!companyId || !fyId) return;
  ...
  const backToGodowns = React.useCallback(() => {
    setLevel({ step: 'godown' });
    setItems([]);
  }, []);
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setGodownIndex((p) => Math.min(godowns.length - 1, p + 1));
        return;
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault();
        setGodownIndex((p) => Math.max(0, p - 1));
        return;
      }
      if (e.key === 'Enter') {
        e.preventDefault();
        const g = godowns[godownIndex];
        if (g) loadItems(g);
        return;
      }
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setItemIndex((p) => Math.min(summaryEntries.length - 1, p + 1));
        return;
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault();
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setGroupItemIndex((p) => Math.min(list.length - 1, p + 1));
        return;
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault();
        setGroupItemIndex((p) => Math.max(0, p - 1));
        return;
      }
      if (e.key === 'Enter') {
        e.preventDefault();
        const it = list[groupItemIndex];
        if (it) loadMonths(level.godown, it, level.group);
        return;
      }
      if (e.key === 'Escape' || e.key === 'Backspace') {
        e.preventDefault();
        setLevel({ step: 'summary', godown: level.godown });
      }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        ...
        if (activeFY?.start_date) {
          ...
          loadVouchers(level.godown, level.item, from, to, level.group);
        } else loadVouchers(level.godown, level.item, undefined, undefined, level.group);
        return;
      }
      if (e.key === 'Escape' || e.key === 'Backspace') {
        e.preventDefault();
        backToSummary(level.godown, level.group);
      }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape' || e.key === 'Backspace') {
        e.preventDefault();
        backToMonthly(level.godown, level.item, level.group);
      }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
      ...
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <div className="flex justify-between items-center px-3 py-1.5 bg-white border-b border-zinc-300 font-mono">
          <div className="flex flex-col gap-0.5">
            <span>
              Godown: <span className="font-bold">{level.godown.name}</span>
            </span>
            {inGroup && (
              <span>
                ...
                summaryEntries.map((entry, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b border-zinc-100 cursor-pointer ${isFocused ? 'bg-[#e4e4e7] text-zinc-950 font-bold' : 'hover:bg-zinc-50 text-zinc-800'}`}
                    >
                      <td className={`px-3 py-1 ${isGroup ? 'font-semibold' : ''}`}>
                        {entry.name}
                      </td>
                      <td className="px-3 py-1 text-right border-l border-zinc-100">
                        {fmtQty(qty, unit)}
        ...
        <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
          <button onClick={backToGodowns} className="hover:underline hover:text-zinc-900">
            F4: Godown
          </button>
        </div>
      </div>
  ...
  const totalInQty =
    voucherOpening.qty + voucherRows.reduce((s, r) => s + (Number(r.inwards_qty) || 0), 0);
  const totalInValue =
    voucherOpening.value + voucherRows.reduce((s, r) => s + (Number(r.inwards_value) || 0), 0);
  ...
  const finalClosingQty = voucherRows.length
    ? voucherRows[voucherRows.length - 1].closing_qty
    : voucherOpening.qty;
  const finalClosingValue = voucherRows.length
    ? voucherRows[voucherRows.length - 1].closing_value
    : voucherOpening.value;
                ...
                {hasOpening && (
                  <tr className="border-b border-zinc-100 italic text-zinc-700">
                    <td className="px-3 py-1 whitespace-nowrap">
                      {formatDate(activeFY?.start_date)}
                    </td>
                    <td className="px-3 py-1">Opening Balance</td>
                    <td className="px-3 py-1" />
                    <td className="px-3 py-1 text-right" />
                    <td className="px-3 py-1 text-right border-l border-zinc-100">
                      {fmtQty(voucherOpening.qty)}
                    </td>
