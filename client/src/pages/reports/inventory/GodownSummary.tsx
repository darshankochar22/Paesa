...
type Level =
  ...
  | { step: "monthly"; godown: GodownRow; item: ItemRow }
  | { step: "vouchers"; godown: GodownRow; item: ItemRow };
...
export default function GodownSummary() {
  ...
  const loadMonths = React.useCallback((godown: GodownRow, item: ItemRow) => {
    if (!companyId || !fyId) return;
    setLevel({ step: "monthly", godown, item });
  ...
  const backToSummary = React.useCallback((godown: GodownRow) => { setLevel({ step: "summary", godown }); setMonths([]); }, []);
  const backToMonthly = React.useCallback((godown: GodownRow, item: ItemRow) => { setLevel({ step: "monthly", godown, item }); setVoucherRows([]); }, []);
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter") { e.preventDefault(); const it = items[itemIndex]; if (it) loadMonths(level.godown, it); return; }
      if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToGodowns(); }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [level, items, itemIndex, loadMonths, backToGodowns]);
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToMonthly(level.godown, level.item); }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // LEVEL 2 — Godown Summary: items in godown (matches screenshot 3)
  // ═══════════════════════════════════════════════════════════════════════
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
          <button onClick={backToGodowns} className="hover:underline hover:text-zinc-900">Q: Quit</button>
