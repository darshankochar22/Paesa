...
export default function TransferAnalysis() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.voucherType.getAll(companyId).then((res: any) => {
      ...
      const list = all.filter(
        (t) => t.name === 'Stock Journal' || t.parent_name === 'Stock Journal',
      );
  ...
  const loadVouchers = React.useCallback(
    (voucherType: string, item: ItemRow) => {
      if (!companyId || !fyId) return;
      ...
      (window as any).api.report
        .transferItemVouchers(companyId, fyId, voucherType, item.item_id)
        .then((res: any) => {
  ...
  const backToReport = React.useCallback((voucherType: string) => {
    setLevel({ step: 'report', voucherType });
    setVouchers([]);
  }, []);
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        const t = filtered[selectIdx];
        if (t) loadReport(t.name);
    ...
    if (level.step === 'report') {
      const h = (e: KeyboardEvent) => {
        if (e.key === 'ArrowDown') {
          ...
          if (it) loadVouchers(level.voucherType, it);
    ...
    if (level.step === 'vouchers') {
      const h = (e: KeyboardEvent) => {
        if (e.key === 'ArrowDown') {
          ...
          e.preventDefault();
          backToReport(level.voucherType);
        }
      };
      window.addEventListener('keydown', h);
      return () => window.removeEventListener('keydown', h);
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-black font-sans text-[11px]">
        ...
        <SelectionPopup
          ...
          fieldLabel="Name of Voucher"
          listLabel="List of Voucher Types"
  ...
  if (level.step === 'report') {
    ...
    return (
      <MovementAnalysisTable
        ...
        leftLabel="Goods In (Production)"
        rightLabel="Goods Out (Consumption)"
        rows={rows}
        loading={loading}
        error={err}
        emptyText="No transfers found for this voucher type."
        ...
        footer={
          <FooterBar>
            <button onClick={backToSelect} className="hover:underline hover:text-black">
              Q: Back to Voucher Type Selection
            </button>
  ...
  return (
    <ItemVoucherAnalysis
      ...
      footer={
        <FooterBar>
          <button onClick={() => backToReport(vt)} className="hover:underline hover:text-black">
            Q: Back
          </button>
