...
export interface MovCursor { section: MovSection; idx: number }
...
export default function ItemMovementAnalysis({
  itemName, companyName, periodLabel, unit, inward, outward,
  loading, error, cursor, onCursor, onActivate, footer,
}: Props) {
  const renderRows = (list: PartyMov[], section: MovSection, sublabel: string) => (
    <>
      <tr>
        <td colSpan={5} className="px-4 py-0.5 italic text-[10px] text-zinc-500">{sublabel}:</td>
      </tr>
      {list.length === 0 ? (
        <tr><td colSpan={5} className="px-6 py-1 text-zinc-400 italic">None</td></tr>
      ) : list.map((r, idx) => {
      ...
      {list.length > 0 && (() => {
        ...
        return (
          <tr className="border-t border-zinc-400 font-bold text-zinc-900">
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
      ...
      <div className="flex justify-between items-center px-3 py-1.5 bg-white border-b border-zinc-300 font-mono text-[11px]">
        <span><span className="text-zinc-500">Stock Item:</span> <span className="font-semibold">{itemName}</span></span>
        <span className="text-zinc-600">Movement Values&nbsp;&nbsp;{periodLabel}</span>
      </div>
            ...
            {loading ? (
              ...
              <>
                <tr className="bg-zinc-50 border-t border-zinc-300">
                  <td colSpan={5} className="px-2 py-1 font-bold text-zinc-900">Movement Inward:</td>
                </tr>
                {renderRows(inward, "inward", "Suppliers")}
                <tr><td colSpan={5} className="py-2" /></tr>
                <tr className="bg-zinc-50 border-t border-zinc-300">
                  <td colSpan={5} className="px-2 py-1 font-bold text-zinc-900">Movement Outward:</td>
                </tr>
