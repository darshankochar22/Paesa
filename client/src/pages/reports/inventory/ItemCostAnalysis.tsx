...
const MODE_META: Record<CostMode, {
  title: string; popupTitle: string; fieldLabel: string; listLabel: string; createPath?: string;
}> = {
  group: { title: "Stock Group Cost Analysis", popupTitle: "Select Stock Group", fieldLabel: "Name of Group", listLabel: "List of Stock Groups", createPath: "/master/create/stock-group" },
  item:  { title: "Stock Item Cost Analysis",  popupTitle: "Select Stock Item",  fieldLabel: "Name of Item",  listLabel: "List of Stock Items",  createPath: "/master/create/stock-item" },
  track: { title: "Cost Track Break-up",       popupTitle: "Select Cost Track",  fieldLabel: "Name of Cost Track", listLabel: "List of Cost Tracks" },
};
...
export default function ItemCostAnalysis({ mode }: { mode: CostMode }) {
  ...
  React.useEffect(() => {
    ...
    if (mode === "track") { setList([]); setListLoading(false); return; }
    setListLoading(true);
    ...
    p.then((res: any) => {
      ...
      const items: Ref[] = raw
        .map((r: any) => mode === "group" ? { id: r.sg_id, name: r.name } : { id: r.item_id, name: r.name })
  ...
  const filtered = React.useMemo(() =>
    search.trim() === "" ? list : list.filter(g => g.name.toLowerCase().includes(search.toLowerCase())),
    [list, search]
  );
  ...
  const loadReport = React.useCallback((ref: Ref) => {
    if (!companyId || !fyId) return;
    setLevel({ step: "report", ref });
    setLoading(true); setErr(null); setRowIdx(0);
    (window as any).api.report.itemCostAnalysis(companyId, fyId, mode, ref.id).then((res: any) => {
  ...
  React.useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (level.step === "select") {
        if (mode === "track") {
          if (e.key === "Enter") { e.preventDefault(); acceptTrack(); }
  ...
  if (level.step === "select") {
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <SelectionPopup
          title={meta.popupTitle} fieldLabel={meta.fieldLabel} listLabel={meta.listLabel}
          companyName={selectedCompany?.name}
          items={filtered.map(g => ({ id: g.id, name: g.name }))}
          index={selectIdx} loading={listLoading} search={search}
          emptyText={mode === "track" ? "Type a cost track name and press Enter." : "No records found."}
          onSearchChange={setSearch} onIndexChange={setSelectIdx}
          onAccept={(i) => { if (mode === "track") acceptTrack(); else { const g = filtered[i]; if (g) loadReport(g); } }}
          onCancel={() => navigate(-1)}
  ...
  const costRows: CostRow[] = rows.map(r => ({
    ...
    cost: r.cost, revenue: r.revenue, balance: r.balance, profit: r.profit,
  }));
  return (
    <CostAnalysisTable
      title={meta.title} companyName={selectedCompany?.name} subtitle={level.ref.name} periodLabel={periodLabel}
      rows={costRows} loading={loading} error={err}
      emptyText="No cost-tracking movements found."
      selectedIndex={rowIdx} onSelectIndex={setRowIdx}
      footer={<FooterBar><button onClick={backToSelect} className="hover:underline hover:text-zinc-900">Q: Back to Selection</button><span className="text-zinc-400">Close Cost Track</span></FooterBar>}
    />
  );
}
