...
interface Entry { ledger_id: number; ledger_name: string; type: "Dr" | "Cr"; amount: number; }
interface StockEntry {
  stock_entry_id: number; item_name: string; quantity: number; rate: number; amount: number;
  godown_name?: string | null; unit_symbol?: string | null;
}
interface BillRef { bill_id: number; bill_name: string; bill_type: string; amount: number; due_date: string; }
interface OrderDetails { source_godown_name: string | null; order_nos: string | null; }
interface Voucher {
  voucher_id: number; voucher_type: string; voucher_number: string; date: string;
  supplier_invoice_no: string | null; supplier_invoice_date: string | null;
  place_of_supply: string | null; narration: string | null;
  party_name: string | null; party_ledger_id: number | null;
  entries: Entry[]; stock_entries: StockEntry[]; bill_references: BillRef[];
...
export default function VoucherDisplay({ voucherId, onClose }: { voucherId: number; onClose: () => void }) {
  ...
  React.useEffect(() => {
    ...
    window.api.voucher.getById(voucherId).then((res: any) => {
      if (!active) return;
      if (res?.success) setVoucher(res.voucher as Voucher);
      else setErr(res?.error ?? "Voucher not found.");
      setLoading(false);
    }).catch((e: any) => { if (active) { setErr(e.message); setLoading(false); } });
    return () => { active = false; };
  }, [voucherId]);
  ...
  React.useEffect(() => {
    ...
    window.api.voucher
      ...
      .then((r: any) => { if (active && r?.success) setBalance(r.balance ?? ""); })
      .catch(() => {});
    return () => { active = false; };
  ...
  React.useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "F" || e.key === "f") { setDetailed((d) => !d); }
    };
  ...
  const acctEntries = (voucher?.entries ?? []).filter((e) => e.ledger_id !== voucher?.party_ledger_id);
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-h-0 bg-white select-none text-zinc-900 font-mono text-[11px]">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-white border-b-2 border-zinc-900">
        ...
        <button onClick={onClose} className="text-sm font-bold text-zinc-500 hover:text-zinc-900 font-sans">&times;</button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-6 py-3">
            ...
            <div className="text-center font-bold text-sm underline mb-3">{voucher.voucher_type}</div>
              ...
              <HRow
                ...
                value={
                  <span>
                    ...
                    <span className="font-semibold">{dmy(voucher.supplier_invoice_date) || "—"}</span>
                  </span>
                }
              />
              <HRow label="GST Registration" value={voucher.place_of_supply ? `${voucher.place_of_supply} Registration` : "Not Applicable"} />
              <HRow label="Excise Unit" value="Not Applicable" />
              <div className="h-2" />
              <HRow label="Party Ledger A/c" value={voucher.party_name || "—"} />
              {balance && <HRow label="Current Balance" value={<span className="italic">{balance}</span>} />}
            </div>
                      ...
                      <tr>
                        <td colSpan={6} className="pb-1 pl-3">
                          ...
                          <div className="pl-3 flex flex-wrap gap-x-6">
                            <span>Godown Name: <span className="font-semibold">{r.godown_name || "Main Location"}</span> {qty(r.quantity, r.unit_symbol)} @ {amt(r.rate)} = {amt(r.amount)}</span>
                          </div>
                          <div className="pl-3 flex flex-wrap gap-x-6 text-zinc-700">
                            <span>Tracking No : Not Applicable</span>
                            {voucher.order_details?.order_nos && <span>Order No : {voucher.order_details.order_nos}</span>}
                          </div>
                        </td>
                      </tr>
                    )}
              ...
              <div className="mt-3">
                ...
                {acctEntries.map((e, i) => (
                  <div key={i} className="pl-3 flex">
                    ...
                    <span className="w-40 text-right">{amt(e.amount)} {e.type}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="mt-3">
                ...
                {voucher.bill_references.map((b) => (
                  <div key={b.bill_id} className="pl-3 flex">
                    ...
                    <span className="w-40 text-right">{amt(b.amount)} {partySide}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="flex items-center gap-6 px-3 py-1 border-t border-zinc-300 bg-white text-[10px] font-semibold text-zinc-600 shrink-0">
        <button onClick={onClose} className="hover:text-zinc-900">Q: Quit</button>
        <button onClick={() => setDetailed((d) => !d)} className="hover:text-zinc-900">F: {detailed ? "Condensed" : "Detailed"}</button>
