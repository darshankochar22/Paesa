...
interface GroupRow { sg_id: number; name: string; }
interface ItemRow {
  item_id: number;
  item_name: string;
  unit_name?: string;
...
export default function StockGroupSummary() {
  ...
  const loadItems = React.useCallback((group: GroupRow) => {
    ...
    (window as any).api.report
      .stockGroupItems(companyId, fyId, group.sg_id)
      .then((res: any) => {
  ...
  const backToGroups = React.useCallback(() => { setLevel({ step: "group" }); setItems([]); }, []);
  const backToSummary = React.useCallback((group: GroupRow) => { setLevel({ step: "summary", group }); setMonths([]); }, []);
  const backToMonthly = React.useCallback((group: GroupRow, item: ItemRow) => { setLevel({ step: "monthly", group, item }); setVoucherRows([]); }, []);

  // ── Keyboard nav ─────────────────────────────────────────────────────────
  React.useEffect(() => {
    if (level.step !== "group") return;
    const handler = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown") { e.preventDefault(); setGroupIndex((p) => Math.min(groups.length - 1, p + 1)); return; }
      if (e.key === "ArrowUp") { e.preventDefault(); setGroupIndex((p) => Math.max(0, p - 1)); return; }
      if (e.key === "Enter") { e.preventDefault(); const g = groups[groupIndex]; if (g) loadItems(g); return; }
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter") { e.preventDefault(); const it = items[itemIndex]; if (it) loadMonths(level.group, it); return; }
      if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToGroups(); }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  ...
  React.useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToMonthly(level.group, level.item); }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
      ...
      <div className="flex h-full w-full items-start justify-center bg-zinc-100 select-none" style={{ fontFamily: "system-ui, sans-serif" }}>
        <div className="flex h-full">
          <div className="flex flex-col w-[360px] border-x border-zinc-300 bg-white">
            <div className="px-3 py-1.5 bg-[#003047] text-white text-sm font-semibold text-center">Select Stock Group</div>
            ...
            <div className="flex items-center gap-2 px-3 py-2 border-b border-zinc-300">
              <span className="text-xs w-28 shrink-0">Name of Group</span>
          ...
          <div className="flex flex-col w-[300px] border-r border-zinc-300 bg-white">
            <div className="px-3 py-1.5 bg-[#003047] text-white text-xs font-semibold text-center">List of Stock Groups</div>
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingGroups ? (
                <div className="px-3 py-4 text-xs text-zinc-400 italic">Loading groups...</div>
              ) : groups.length === 0 ? (
                <div className="px-3 py-4 text-xs text-zinc-400 italic">No stock groups found.</div>
              ) : (
                groups.map((g, idx) => {
                  const isFocused = idx === groupIndex;
                  return (
                    <div
                      key={g.sg_id}
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <div className="flex justify-between items-center px-3 py-1.5 bg-white border-b border-zinc-300 font-mono">
          <span>Stock Group: <span className="font-bold">{level.group.name}</span></span>
        ...
        <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
          <button onClick={backToGroups} className="hover:underline hover:text-zinc-900">Q: Quit</button>
          <button onClick={backToGroups} className="hover:underline hover:text-zinc-900">F4: Stock Group</button>
        </div>
      </div>
    );
  }
