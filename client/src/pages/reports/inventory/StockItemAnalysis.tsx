...
export default function StockItemAnalysis() {
  ...
  const openVouchers = React.useCallback((section: MovSection, party: string) => {
    setLevel((l) =>
      l.step === 'movement' ? { step: 'vouchers', item: l.item, party, direction: section } : l,
    );
    setVoucherIdx(0);
  }, []);
  ...
  return (
    <ItemVoucherAnalysis
      ...
      footer={
        <FooterBar>
          <button onClick={() => loadMovement(it)} className="hover:underline hover:text-black">
            Q: Back
          </button>
