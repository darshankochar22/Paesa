...
export default function StockAgeingTable({
  ...
  loading,
  error,
  emptyText = 'No stock in this group.',
  selectedIndex,
  onSelectIndex,
  onActivate,
  footer,
  ...
  const totals = rows.reduce(
    (acc, r) => {
      acc.total.qty += r.total.qty;
      acc.total.value += r.total.value;
      r.buckets.forEach((b, i) => {
        acc.buckets[i].qty += b.qty;
        acc.buckets[i].value += b.value;
      });
      acc.neg.qty += r.neg.qty;
      acc.neg.value += r.neg.value;
      return acc;
    },
    {
      total: { qty: 0, value: 0 },
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-black font-sans text-[11px]">
      ...
      <div className="flex justify-between items-center px-3 py-1 bg-white border-b border-gray-200 font-mono text-[11px]">
      ...
      {basis ? (
        <div className="flex justify-between items-center px-3 py-1 bg-white border-b border-gray-200 font-mono text-[11px]">
          ...
          <span className="text-[10px] italic text-black">Valued based on Actual Transfer</span>
        </div>
      ) : (
        <div className="flex justify-between items-center px-3 py-1 bg-white border-b border-gray-200 font-mono text-[10px] italic text-black">
        ...
        <table className="w-full border-collapse text-[11px] font-mono select-none">
          <thead className="sticky top-0 bg-white border-b border-gray-200 z-10 text-black">
            <tr>
              ...
              <th
                rowSpan={2}
                className="px-2 py-1 text-left font-bold align-bottom border-b border-l border-gray-200 w-24"
              >
                Expiry Date
              </th>
              {groups.map((g) => (
                <th
                  key={g.key}
                  colSpan={g.qtyOnly ? 1 : 2}
                  className="px-2 py-0.5 text-center font-bold border-b border-l border-gray-200 whitespace-nowrap"
                >
                  {g.label}
                </th>
              ))}
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={colCount} className="px-4 py-8 text-center text-black italic">
                  Loading...
                </td>
              </tr>
            ) : error ? (
              <tr>
                <td colSpan={colCount} className="px-4 py-8 text-center text-black">
                  {error}
                </td>
              </tr>
            ) : rows.length === 0 ? (
              ...
              rows.map((r, idx) => (
                ...
                >
                  ...
                  <td className="px-2 py-1 border-l border-gray-200">{r.expiry || ''}</td>
                  {groups.map((g) => {
                    const c = g.cell(r);
                    return (
                      <React.Fragment key={g.key}>
          ...
          {!loading && !error && rows.length > 0 && (
            <tfoot className="sticky bottom-0 z-10 bg-white text-black font-bold">
                ...
                {groups.map((g) => {
                  ...
                  const c =
                    g.key === 'total'
                      ...
                      : g.key === 'neg'
                        ? totals.neg
                        : totals.buckets[Number(g.key.slice(1))];
                  return (
                    <React.Fragment key={g.key}>
                      <td className="px-2 py-1.5 text-right border-l border-gray-200" />
