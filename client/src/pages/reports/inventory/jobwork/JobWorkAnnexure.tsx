  ...
  const loadReport = React.useCallback(
    (u: ExciseUnit) => {
      ...
      (window as any).api.report
        .jobWorkAnnexure(companyId, fyId, annexure, u.id)
        .then((res: any) => {
  ...
  if (step === 'select') {
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-black font-sans text-[11px]">
        ...
        <SelectionPopup
          title={`Jobwork Annexure - ${annexure}`}
          fieldLabel="Name of excise unit"
          listLabel="List of Excise Units"
          companyName={selectedCompany?.name}
          items={filtered.map((u) => ({ id: u.id, name: u.name }))}
          index={selectIdx}
          loading={unitsLoading}
          search={search}
          emptyText="No excise units found."
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-black font-sans text-[11px]">
      ...
      <div className="border-t-2 border-black bg-white px-3 py-1.5 flex font-mono text-[11px] font-bold text-black shrink-0">
        ...
        <span className="w-32 text-right border-l border-gray-200 pr-2">{fmtQty(totalQty)}</span>
        <span className="w-36 text-right border-l border-gray-200 pr-2">{fmtAmt(totalAmt)}</span>
      </div>
