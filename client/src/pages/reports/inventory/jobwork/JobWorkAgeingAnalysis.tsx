...
export default function JobWorkAgeingAnalysis() {
  ...
  const loadReport = React.useCallback((g: GroupRef, dir: "in" | "out") => {
    if (!companyId || !fyId) return;
    setGroup(g); setStep("report"); setDirection(dir);
    setLoadingReport(true); setReportErr(null); setRowIdx(0);
    (window as any).api.report.jobWorkAgeing(companyId, fyId, g.group_id, asAt, fyStart, dir).then((res: any) => {
      if (res.success) {
        setRows(res.rows ?? []);
  ...
  React.useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (step === "select") {
        ...
        else if (e.key === "Enter") { e.preventDefault(); const g = filtered[selectIdx]; if (g) loadReport(g, direction); }
        ...
        else if (e.key === "F8") { e.preventDefault(); if (group) loadReport(group, direction === "in" ? "out" : "in"); }
        else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToSelect(); }
      }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  }, [step, filtered, selectIdx, rows.length, group, direction, loadReport, backToSelect, navigate]);
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <SelectionPopup
          ...
          onAccept={(i) => { const g = filtered[i]; if (g) loadReport(g, direction); }}
          onCancel={() => navigate(-1)}
  ...
  return (
    <StockAgeingTable
      companyName={selectedCompany?.name} groupLabel={group?.group_name || "Primary"}
      asAt={reportAsAt} basis={direction === "out" ? "Job Work Out" : "Job Work In"}
      bands={bands} rows={ageRows} loading={loadingReport} error={reportErr}
      ...
      footer={
        <FooterBar>
          ...
          <button onClick={() => group && loadReport(group, direction === "in" ? "out" : "in")} className="hover:underline hover:text-zinc-900">
