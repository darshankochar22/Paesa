...
interface Row { item_id: number; item_name: string; unit_name: string; ordered_qty: number; balance_qty: number; }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
      ...
      <div className="flex justify-between items-center px-3 py-1.5 bg-white border-b border-zinc-300 font-mono">
        <span className="font-semibold">{kindLabel}: {basis}</span>
        ...
        <table className="w-full border-collapse text-[11px] font-mono select-none">
          <thead className="sticky top-0 bg-[#f4f4f5] border-b border-zinc-300 z-10 text-zinc-700">
            <tr>
              ...
              <th className="px-3 py-1 text-right font-bold w-44 border-l border-zinc-200">Ordered Quantity</th>
              <th className="px-3 py-1 text-right font-bold w-44 border-l border-zinc-200">Balance Quantity</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={3} className="px-4 py-8 text-center text-zinc-600">{error}</td></tr>
            ) : rows.length === 0 ? (
              ...
              rows.map((r, i) => {
                ...
                return (
                  <tr key={r.item_id} onClick={() => setIdx(i)}
                    ...
                    <td className="px-3 py-1 text-right border-l border-zinc-100">{fmtQty(r.balance_qty, r.unit_name)}</td>
                  </tr>
                );
              })
            )}
      ...
      <div className="border-t-2 border-zinc-300 bg-[#f4f4f5] px-3 py-1.5 flex font-mono text-[11px] font-bold text-zinc-900 shrink-0">
        ...
        <span className="w-44 text-right border-l border-zinc-300 pr-2">{fmtQty(totalOrdered)}</span>
        <span className="w-44 text-right border-l border-zinc-300 pr-2">{fmtQty(totalBalance)}</span>
      </div>
