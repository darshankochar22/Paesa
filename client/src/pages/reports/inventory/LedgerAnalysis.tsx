...
export default function LedgerAnalysis() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.ledger.getAll(companyId).then((res: any) => {
      const list: Ledger[] = [...(res.ledgers ?? [])].sort((a: Ledger, b: Ledger) =>
  ...
  const filtered = React.useMemo(
    () =>
      search.trim() === ''
        ? ledgers
        : ledgers.filter((l) => l.name.toLowerCase().includes(search.toLowerCase())),
    [ledgers, search],
  );
  React.useEffect(() => {
    setSelectIdx(0);
  ...
  const loadReport = React.useCallback(
    (ledger: Ledger) => {
      ...
      (window as any).api.report
        .ledgerAnalysis(companyId, fyId, ledger.ledger_id)
        .then((res: any) => {
  ...
  const loadVouchers = React.useCallback(
    (ledger: Ledger, item: ItemRow) => {
      ...
      setVIdx(0);
      (window as any).api.report
        .ledgerItemVouchers(companyId, fyId, ledger.ledger_id, item.item_id)
        .then((res: any) => {
          if (res.success) {
  ...
  const backToReport = React.useCallback((ledger: Ledger) => {
    setLevel({ step: 'report', ledger });
    setVouchers([]);
  }, []);
    ...
    if (level.step === 'report') {
      const h = (e: KeyboardEvent) => {
        if (e.key === 'ArrowDown') {
          ...
          if (it) loadVouchers(level.ledger, it);
  ...
  if (level.step === 'report') {
    ...
    return (
      <MovementAnalysisTable
        ...
        loading={loading}
        error={err}
        emptyText="No inventory movement found for this ledger."
  ...
  return (
    <ItemVoucherAnalysis
      ...
      footer={
        <FooterBar>
          <button onClick={() => backToReport(l)} className="hover:underline hover:text-black">
            Q: Back to Ledger
          </button>
