...
export default function LedgerPanel({
  ...
  if (isStockItem) {
    ...
    itemsList = godowns.filter(
      (godown) => !searchTerm || godown.name.toLowerCase().includes(searchTerm.toLowerCase()),
    );
  } else {
    ...
    if (activeField?.type === 'particular') {
      ...
      tempLedgers = ledgers.filter((l) =>
        checkLedgerGroup(l, voucherType === 'Sales' ? ['sales accounts'] : ['purchase accounts']),
      );
    }

    itemsList = tempLedgers.filter(
      (l) =>
        !searchTerm ||
  ...
  return (
    <div className="w-80 border-l border-zinc-200 flex flex-col shrink-0 bg-white shadow-lg animate-fade-in font-sans">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 divide-y divide-zinc-100">
        ...
        {loading && <div className="px-3 py-3 text-xs text-zinc-400 italic">Loading list...</div>}
        ...
        {!loading &&
          itemsList.map((item, idx) => {
            ...
            const led = item as LedgerType;
            const balance = led.closing_balance || led.opening_balance;
            // closing/opening balance is stored positive on the ledger's natural side,
            // so derive Dr/Cr from nature — a bare `balance > 0 ? 'Dr'` mislabels every
            // Income/Liability (Sales, Capital, creditors) balance as "Dr".
            const isDrNature = led.nature !== 'Liabilities' && led.nature !== 'Income';
            const balanceDisplay = balance
              ? `${Math.abs(balance).toLocaleString('en-IN', { maximumFractionDigits: 2 })} ${balance > 0 === isDrNature ? 'Dr' : 'Cr'}`
              : '';
            return (
              <div
              ...
              >
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  {item.alias && (
                    <span
                      className={`text-[10px] truncate ${isSelected ? 'text-zinc-300' : 'text-zinc-400'}`}
                    >
                      ({item.alias})
                    </span>
                  )}
                  {!isStockItem && !isGodown && balanceDisplay && (
                    <span
                      className={`text-[10px] ${isSelected ? 'text-zinc-300' : 'text-zinc-500'} font-sans italic`}
                    >
                      Bal: {balanceDisplay}
                    </span>
                  )}
                ...
                {!isStockItem && !isGodown && item.group_name && (
                  <span
                    className={`text-[10px] shrink-0 ml-2 font-sans px-1.5 py-0.5 rounded ${
