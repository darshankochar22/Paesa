...
export default function OrderItemAllocationPopup({
  ...
  const [created, setCreated] = useState<{ tracking: string[]; order: string[]; batch: string[] }>({
    tracking: [],
    order: [],
    batch: [],
  });
  ...
  const [trackPos, setTrackPos] = useState<{ top: number; left: number; width: number } | null>(
    null,
  );
  const [orderPos, setOrderPos] = useState<{ top: number; left: number; width: number } | null>(
    null,
  );
  const [godownPos, setGodownPos] = useState<{ top: number; left: number; width: number } | null>(
    null,
  );
  const [batchPos, setBatchPos] = useState<{ top: number; left: number; width: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      ...
      if (el && !el.contains(target) && !batchDropdownRef.current?.contains(target))
        setOpenListRow(null);
    };
  ...
  const sessionTracking: { no: string; godown?: string; balance?: number }[] = [
    ...fetchedTracking.map((t) => ({
      no: t.name,
      godown: t.godown ?? undefined,
      balance: t.balance,
    })),
    ...mergeNos(
      ...
      .map((no) => ({
        no,
        godown: rows.find((r) => r.tracking_no === no)?.godown,
        balance: undefined,
      })),
  ];
  const sessionOrders: {
    ...
    ...fetchedOrders.map((o) => ({
      no: o.name,
      godown: o.godown ?? undefined,
      due: o.due_on ?? undefined,
      balance: o.balance,
      rate: o.rate,
    })),
    ...mergeNos(
  ...
  const selectOrder = (
    i: number,
    o: { no: string; godown?: string; due?: string; balance?: number; rate?: number },
  ) => {
    update(i, {
      order_no: o.no,
  ...
  const saveAllocations = useCallback(
    (list: BatchAllocation[], skipNegCheck = false) => {
      ...
      if (orphaned.length) {
        ...
        setError(
          `Row${orphaned.length > 1 ? 's' : ''} ${nums} ha${orphaned.length > 1 ? 've' : 's'} only Rate / Disc filled — add a quantity or godown, or clear the row.`,
        );
        return;
      }
  ...
  const inputCls =
    'text-xs px-1 py-0.5 border border-gray-400 bg-white w-full outline-none focus:border-black';
  const smallInputCls =
    'text-[11px] px-1 py-0.5 border border-gray-400 bg-white outline-none focus:border-black font-mono';
  const listHeadCls = 'bg-white text-black font-bold border-b border-gray-300';
  const optNew =
    'block w-full text-right text-[11px] px-2 py-1 hover:bg-gray-100 font-semibold border-b border-gray-100';
  ...
  return (
    <>
                              ...
                              {godowns.map((g) => (
                                ...
                                >
                                  ...
                                  <div className="w-14 text-right font-mono text-gray-600">
                                    {fmtQty(
                                      g.godown_id != null ? godownBal[g.godown_id] : undefined,
                                      unitSymbol,
                                    )}
                                  </div>
                                </button>
      ...
      {newNumber && (
        <NewNumberPopup
          title={
            newNumber.field === 'order'
              ? 'New Order Number'
              : newNumber.field === 'batch'
          ...
          onConfirm={(value) => {
            ...
            setNewNumber(null);
            setCreated((c) =>
              c[field].includes(value) ? c : { ...c, [field]: [...c[field], value] },
            );
            if (field === 'tracking') {
              update(row, { tracking_no: value });
              focusSel(`[data-oa-order="${row}"]`);
            } else if (field === 'order') {
              update(row, { order_no: value });
              focusSel(`[data-oa-due="${row}"]`);
            } else {
              update(row, { batch_number: value });
              focusSel(`[data-oa-actual="${row}"]`);
            }
          }}
        />
      )}
