...
export default function PayrollVoucher({ form }: Props) {
  ...
  // ── focus helpers ──────────────────────────────────────────────────────────
  const focusCategory = (groupId: string) => setTimeout(() => {
    (document.querySelector(`[data-payroll-cat="${groupId}"]`) as HTMLInputElement | null)?.focus();
  }, 50);

  const focusEmployee = (groupId: string, empRowId: string) => setTimeout(() => {
    (document.querySelector(`[data-payroll-emp="${groupId}-${empRowId}"]`) as HTMLInputElement | null)?.focus();
  }, 50);

  const focusPayHead = (groupId: string, empRowId: string, phRowId: string) => setTimeout(() => {
    (document.querySelector(`[data-payroll-ph="${groupId}-${empRowId}-${phRowId}"]`) as HTMLInputElement | null)?.focus();
  }, 50);
  ...
  const focusLastPayHead = (groupId: string, empRowId: string) => setTimeout(() => {
  ...
  const focusAmount = (groupId: string, empRowId: string, phRowId: string) => setTimeout(() => {
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 bg-white">
        {groups.map((group: any) => {
          const isCatActive = form.activeField?.type === "payrollCategory" &&
            (form.activeField as any).groupId === group.id;

          const groupTotal = group.employeeRows.reduce((gSum: number, e: any) =>
            gSum + e.payHeadRows.reduce((eSum: number, ph: any) => eSum + (Number(ph.amountRaw) || 0), 0)
          , 0);

          return (
            <div key={group.id}>
              {/* Category row */}
              <div className="flex items-center border-b border-zinc-100 min-h-[24px] px-3 py-0.5 bg-zinc-50">
                <input
                  data-payroll-cat={group.id}
                  type="text"
                  className="flex-1 text-xs bg-transparent outline-none px-0 border border-transparent focus:border-zinc-800 font-semibold text-zinc-800"
                  value={isCatActive ? form.ledgerSearchTerm : (group.category?.name ?? "")}
                  placeholder="Select Category…"
                  onFocus={() => form.handleFieldFocus({ type: "payrollCategory", groupId: group.id } as any)}
                  onChange={(e) => {
                    form.setLedgerSearchTerm(e.target.value);
                    if (!group.category) form.handleFieldFocus({ type: "payrollCategory", groupId: group.id } as any);
                  }}
                  onKeyDown={(e) => {
                ...
                <span className="flex items-center ml-auto shrink-0">
                  <span className="w-28 text-right text-xs font-semibold text-zinc-800 tabular-nums px-1">
                    {groupTotal > 0 ? groupTotal.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : ""}
                  </span>
                  <span className="w-7" />
                </span>
              </div>
              ...
              {group.employeeRows.map((empRow: any) => {
                const isEmpActive = form.activeField?.type === "payrollEmployee" &&
                  (form.activeField as any).groupId === group.id &&
                  (form.activeField as any).empRowId === empRow.id;

                const empTotal = empRow.payHeadRows.reduce(
                  (s: number, ph: any) => s + (Number(ph.amountRaw) || 0), 0
                );

                return (
                  <div key={empRow.id}>
                    {/* Employee name row */}
                    <div className="flex items-center border-b border-zinc-100 min-h-[22px] px-3 pl-8 py-0.5">
                      <input
                        data-payroll-emp={`${group.id}-${empRow.id}`}
                        type="text"
                        className="flex-1 text-xs bg-transparent outline-none px-0 border border-transparent focus:border-zinc-800 font-semibold"
                        value={isEmpActive ? form.ledgerSearchTerm : (empRow.employee?.name ?? "")}
                        placeholder="Select Employee…"
                        onFocus={() => form.handleFieldFocus({ type: "payrollEmployee", groupId: group.id, empRowId: empRow.id } as any)}
                        onChange={(e) => {
                          form.setLedgerSearchTerm(e.target.value);
                          if (!empRow.employee) form.handleFieldFocus({ type: "payrollEmployee", groupId: group.id, empRowId: empRow.id } as any);
                        }}
                        onKeyDown={(e) => {
                      ...
                      {empTotal > 0 && (
                        <span className="flex items-center ml-auto shrink-0">
                          <span className="w-28 text-right text-xs font-semibold font-mono text-zinc-800 px-1">
                            {empTotal.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </span>
                          <span className="w-7 text-right text-xs font-semibold text-zinc-600">Dr</span>
                        </span>
                      )}
                    </div>
                    ...
                    {empRow.payHeadRows.map((phRow: any, phIdx: number) => {
                      const isPhActive = form.activeField?.type === "payrollPayHead" &&
                        (form.activeField as any).groupId === group.id &&
                        (form.activeField as any).empRowId === empRow.id &&
                        (form.activeField as any).phRowId === phRow.id;

                      return (
                        <div key={phRow.id} className="flex items-center border-b border-zinc-50 min-h-[20px] px-3 pl-16 py-0.5">
                          <input
                            data-payroll-ph={`${group.id}-${empRow.id}-${phRow.id}`}
                            type="text"
                            className="w-48 text-xs bg-transparent outline-none px-0 border border-transparent focus:border-zinc-800 text-zinc-700"
                            value={isPhActive ? form.ledgerSearchTerm : (phRow.payHead?.name ?? "")}
                            placeholder={empRow.employee ? "Select Pay Head…" : ""}
                            onFocus={() => form.handleFieldFocus({ type: "payrollPayHead", groupId: group.id, empRowId: empRow.id, phRowId: phRow.id } as any)}
                            onChange={(e) => {
                              form.setLedgerSearchTerm(e.target.value);
                              if (!phRow.payHead) form.handleFieldFocus({ type: "payrollPayHead", groupId: group.id, empRowId: empRow.id, phRowId: phRow.id } as any);
                            }}
                            onKeyDown={(e) => {
                          ...
                          <input
                            ...
                            type="text"
                            inputMode="decimal"
                            className="w-28 text-right text-xs bg-transparent outline-none px-1 border border-transparent focus:border-zinc-800 font-mono font-semibold ml-auto"
                            value={phRow.amountRaw}
                            placeholder={phRow.payHead ? "0.00" : ""}
                            onChange={(e) => (form as any).handleUpdatePayrollPayHeadRow(group.id, empRow.id, phRow.id, { amountRaw: e.target.value })}
                            onKeyDown={(e) => {
                              if (e.key === "Enter") {
                                e.preventDefault();
                          ...
                          />
                          <span className="w-7 text-right text-xs font-semibold text-zinc-600 shrink-0">
        ...
        {Array.from({ length: Math.max(0, 8 - groups.length) }).map((_: any, i: number) => (
          <div key={`pay-f-${i}`} className="flex border-b border-zinc-50 min-h-[22px] px-3" />
        ))}
      </div>

      {/* Grand total footer */}
      <div className="flex border-t border-black shrink-0 px-3 py-1 bg-zinc-50">
        <div className="flex-1 text-xs font-bold text-zinc-700">Total</div>
        <div className="w-28 text-right text-xs font-bold font-mono text-zinc-900 px-1">
          {form.totalAmount > 0
            ? form.totalAmount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
            : ""}
        </div>
        <div className="w-7 text-right text-xs font-bold text-zinc-700">Dr</div>
      </div>
    </>
  );
}
