import {
  type Voucher,
  formatDate,
  ReadOnlyFieldRow, ReadOnlyStockTable, ReadOnlySplitStockTable,
  ReadOnlyDoubleEntryTable, ReadOnlyBillReferences,
  STOCK_TABLE_VARIANT,
} from "./shared";
...
export default function InventoryVoucherView({ voucher, balances }: { voucher: Voucher; balances: Record<number, string> }) {
  ...
  const sourceGodownRowLabel = t === "Material In" ? "Source Godown"
    : t === "Material Out" ? "Destination Godown" : null;
  ...
  return (
    <>
      ...
      {["Sales Order", "Purchase Order", "Job Work In Order"].includes(t) && voucher.order_details?.order_nos && (
