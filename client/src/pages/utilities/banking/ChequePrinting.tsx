...
// TallyPrime Banking → Cheque Printing register.
export default function ChequePrinting() {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await window.api.banking.getChequePrinting(
        companyId,
        fyId,
        ledgerId,
        fromDate || undefined,
      ...
      if (res.success) {
        ...
        setError(res.error || 'Failed to load cheques');
      }
    } catch (e: any) {
      setError(e?.message || 'Failed to load cheques');
    } finally {
      setLoading(false);
    }
  ...
  const printSelected = useCallback(async () => {
    const ids = [...selected];
    if (ids.length === 0) {
      setError('Select cheques with Space before printing.');
      return;
    }
  ...
  const rightActions: RightPanelAction[] = [
    ...
    { key: 'B', label: 'Alter Ledger', onClick: () => ledgerId && navigate(`/master/alter`) },
    {
      key: 'H',
    ...
    <div className="fixed inset-0 z-40 flex flex-col bg-white font-sans text-black">
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!isConfigured && ledgerId && (
            <div className="text-center italic font-semibold text-[11px] py-1 border-b border-gray-200">
                  ...
                  <tr>
                    <td colSpan={7} className="px-2 py-8 text-center text-gray-500 italic">
                      {loading
                        ? 'Loading…'
                        : ledgerId
                          ? 'No cheques for this bank in the selected period.'
                          : 'Select a bank.'}
                    </td>
                  </tr>
                ) : (
        ...
        <RightActionPanel actions={rightActions} title="Cheque Printing" />
      </div>
            ...
            <div className="max-h-72 overflow-auto">
              {filteredBanks.length === 0 ? (
                ...
                filteredBanks.map((b) => (
                  ...
                  >
                    <span>{b.name}</span>
                    {!b.enable_cheque_printing && (
                      <span className="text-[8px] uppercase text-gray-400">not configured</span>
                    )}
                  </button>
                ))
              )}
