...
import {
  computeInterstate,
  validateTaxLedgerSelection,
} from '../pages/transactions/utils/interstate';

describe('computeInterstate', () => {
  it('same state = intra-state', () => {
    expect(computeInterstate({ companyState: 'Chhattisgarh', placeOfSupply: 'Chhattisgarh' })).toBe(
      false,
    );
  });
  it('different states = inter-state', () => {
    expect(computeInterstate({ companyState: 'Chhattisgarh', placeOfSupply: 'Maharashtra' })).toBe(
      true,
    );
  });
  it('GSTIN prefix wins over the state name (like the server)', () => {
    ...
    expect(
      computeInterstate({ companyGstin: '22AAMCS8857L1ZM', partyGstin: '27ABCDE1234F1Z5' }),
    ).toBe(true);
  });
...
describe('validateTaxLedgerSelection (block at selection time)', () => {
  ...
  it('ALLOWS CGST/SGST on a same-state supply', () => {
    expect(
      validateTaxLedgerSelection({ name: 'CGST', gst_tax_type: 'CGST' }, chhattisgarhSupply),
    ).toBeNull();
    expect(
      validateTaxLedgerSelection({ name: 'SGST', gst_tax_type: 'SGST/UTGST' }, chhattisgarhSupply),
    ).toBeNull();
  });

  it('BLOCKS CGST/SGST on a different-state (inter) supply', () => {
    expect(
      validateTaxLedgerSelection({ name: 'CGST', gst_tax_type: 'CGST' }, interStateSupply),
    ).toMatch(/IGST/);
    expect(
      validateTaxLedgerSelection({ name: 'SGST', gst_tax_type: 'SGST/UTGST' }, interStateSupply),
    ).toMatch(/IGST/);
  });
  ...
  it('BLOCKS a second CGST but ALLOWS the matching SGST', () => {
    ...
    expect(
      validateTaxLedgerSelection(
        { name: 'CGST', gst_tax_type: 'CGST' },
        chhattisgarhSupply,
        existing,
      ),
    ).toMatch(/already added/i);
    expect(
      validateTaxLedgerSelection(
        { name: 'SGST', gst_tax_type: 'SGST/UTGST' },
        chhattisgarhSupply,
        existing,
      ),
    ).toBeNull();
  ...
  it('BLOCKS IGST when CGST/SGST already present, even if state is unknown', () => {
    const existing = [
      { name: 'CGST', gst_tax_type: 'CGST' },
      { name: 'SGST', gst_tax_type: 'SGST/UTGST' },
    ];
    const err = validateTaxLedgerSelection({ name: 'IGST' }, {}, existing);
    expect(err).toMatch(/can't have both/i);
  });

  it('BLOCKS CGST/SGST when IGST already present, even if state is unknown', () => {
    const existing = [{ name: 'IGST' }];
    expect(
      validateTaxLedgerSelection({ name: 'CGST', gst_tax_type: 'CGST' }, {}, existing),
    ).toMatch(/can't have both/i);
    expect(
      validateTaxLedgerSelection({ name: 'SGST', gst_tax_type: 'SGST/UTGST' }, {}, existing),
    ).toMatch(/can't have both/i);
  });
});
