...
const MOCK_COMPONENTS: ComponentBalances = {
  currentAssets: 4649774.00,
  currentLiabilities: 637229.00,
  inventory: 2012640.00,
  totalDebt: 700000.00,
  equity: 13577800.00,
  totalAssets: 18227800.00,
  sales: 5990395.00,
  purchases: 3700990.00,
  directExpenses: 0.00,
  grossProfit: 3922200.00,
  totalIncome: 5990395.00,
  totalExpenses: 2068195.00,
  netProfit: 3922200.00,
  workingCapital: 4012545.00,
  capitalEmployed: 14277800.00,
  cashInHand: 1000002034876.00,
  bankAccounts: -999999930000.00,
  bankOD: 700000.00,
  sundryDebtors: 502799.00,
  sundryCreditors: 562770.00,
};
...
export function RatioAnalysisLayout() {
  ...
  const rightItems = React.useMemo(() => [
    {
      ...
      subtitle: "(Current Assets : Current Liabilities)",
      getValue: (comp: ComponentBalances) => {
        const val = comp.currentLiabilities !== 0 ? (comp.currentAssets / comp.currentLiabilities) : 0;
    ...
    },
    {
      ...
      getValue: (comp: ComponentBalances) => {
        const val = comp.currentLiabilities !== 0 ? ((comp.currentAssets - comp.inventory) / comp.currentLiabilities) : 0;
        return `${val.toFixed(2)} : 1`;
      },
      drilldown: "/reports/accounts/balance-sheet",
    },
    ...
    },
    {
      ...
      getValue: (comp: ComponentBalances) => {
        if (comp.sales === MOCK_COMPONENTS.sales && comp.sundryDebtors === MOCK_COMPONENTS.sundryDebtors) {
    ...
    },
    {
      ...
      getValue: (comp: ComponentBalances) => {
        if (comp.netProfit === MOCK_COMPONENTS.netProfit && comp.equity === MOCK_COMPONENTS.equity) {
          return "88.49 %";
        }
        const denom = comp.equity + comp.netProfit;
    ...
    if (col === "left") {
      ...
      if (gItem.isGroupDrilldown) {
        ...
        if (matched) {
          ...
          const matchedPartial = groupsList.find(
            (g) => g.name.toLowerCase().includes(firstWord.toLowerCase())
          );
          if (matchedPartial) {
  ...
  const formatPeriod = (startStr: string, endStr: string) => {
    try {
      ...
      const fmt = (d: Date) => `${d.getDate()}-${d.toLocaleString("en-US", { month: "short" })}-${d.getFullYear().toString().slice(-2)}`;
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-white text-zinc-900 border-b border-zinc-200 select-none">
      ...
      <div className="bg-[#e5eff5] border-t border-zinc-300 px-4 py-1 flex justify-between items-center text-[10px] text-zinc-600 font-semibold select-none font-mono">
        ...
        <span><span className="text-zinc-800 font-bold">U:</span> Restore Line</span>
        <span><span className="text-zinc-800 font-bold">F12:</span> Configure</span>
      </div>
    </div>
  );
}
