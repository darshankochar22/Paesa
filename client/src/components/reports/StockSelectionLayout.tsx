...
export default function StockItemSelectionLayout() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (i) => i.name.toLowerCase().includes(q) || (i.alias && i.alias.toLowerCase().includes(q)),
    );
  }, [stockItems, search]);
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setSelectedIndex((p) => Math.min(p + 1, totalRows - 1));
        return;
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault();
  ...
  if (activeView.mode === 'alter') {
    return (
      <div className="h-full w-full">
        <StockItemAlter
          initialItemId={activeView.itemId}
          onDone={backToList}
          onCancel={backToList}
        />
      </div>
  ...
  return (
    <div
      className="flex h-full w-full items-start justify-center bg-white select-none"
      style={{ fontFamily: 'system-ui, sans-serif' }}
    >
      <div className="flex flex-col w-[380px] h-full border-x border-gray-200 bg-white">
        <div className="px-3 py-2 border-b border-gray-200">
          ...
          <input
            ...
            className="w-full border border-gray-200 px-2 py-1 text-sm outline-none focus:border-gray-200 bg-white"
            placeholder=""
          />
        </div>

        <div className="bg-black text-white text-sm font-semibold px-3 py-1">
          List of Stock Items
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div
            onClick={goToCreate}
            className={`px-3 py-1 text-sm cursor-pointer border-b border-gray-200 ${
              selectedIndex === 0 ? 'bg-black/[0.06] font-semibold' : 'hover:bg-black/[0.03]'
            }`}
          >
            Create
          </div>
          ...
          {loading ? (
            ...
            <div className="px-3 py-4 text-xs text-black italic">No stock items found.</div>
          ) : (
            filtered.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  className={`px-3 py-1 text-sm cursor-pointer border-b border-gray-200 ${
                    isSelected ? 'bg-black/[0.06] font-semibold' : 'hover:bg-black/[0.03]'
                  }`}
                >
                  {item.name}
                </div>
