...
export default function LedgerOutstandingsLayout() {
      ...
      <div className="flex flex-col h-full w-full bg-white font-mono overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          <table className="w-full border-collapse text-[11px] font-mono">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={2} className="px-4 py-8 text-center text-zinc-400 italic">No ledgers found.</td></tr>
              ) : filtered.map((l, idx) => (
                <tr
                  key={l.ledger_id}
  ...
  if (loading) return <div className="flex-1 flex items-center justify-center text-zinc-400 font-mono text-xs">Loading Ledger Outstandings...</div>;
    ...
    <div className="flex flex-col h-full w-full bg-white font-mono overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        <table className="w-full border-collapse text-[11px] font-mono">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-zinc-400 italic">No pending bills for this ledger.</td></tr>
            ) : rows.map((row, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={`px-3 py-1.5 text-center ${row.overdue_days > 0 ? "text-zinc-700 font-bold" : ""}`}>{row.overdue_days > 0 ? row.overdue_days : ""}</td>
