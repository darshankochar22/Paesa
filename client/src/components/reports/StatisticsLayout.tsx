...
const ColHeader = ({
  ...
  <div className="bg-white sticky top-0 px-3 py-1 border-b border-gray-200 select-none flex items-start justify-between gap-2 text-[11px]">
    <div className="font-bold text-black">{title}</div>
    <div className="text-right leading-tight">
      <div className="font-bold text-black">{companyName}</div>
      <div className="text-black text-[10px]">{periodText}</div>
    </div>
  </div>
);
...
export default function StatisticsLayout() {
  ...
  const periodText = activeFY ? `${fmt(activeFY.start_date)} to ${fmt(activeFY.end_date)}` : '';
  ...
  const accountsRows = React.useMemo(
    () => [
      ...
      {
        name: 'Cost Centres',
        count: data?.accounts?.costCentres || 0,
        path: '/master/coa/cost-centre',
      },
      {
        name: 'Stock Groups',
        count: data?.accounts?.stockGroups || 0,
        path: '/master/coa/stock-group',
      },
      {
        name: 'Stock Categories',
        count: data?.accounts?.stockCategories || 0,
        path: '/master/coa/stock-category',
      },
      {
        name: 'Stock Items',
        count: data?.accounts?.stockItems || 0,
        path: '/master/alter/stock-item',
      },
      {
        name: 'Voucher Types',
        count: data?.accounts?.voucherTypes || 0,
        path: '/master/coa/voucher-type',
      },
      ...
      { name: 'Attendance/Production Types', count: 0, path: '/master/coa/attendance-type' },
      {
        ...
        count: data?.accounts?.employeeGroups || 0,
        path: '/master/coa/employee-group',
      },
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white font-mono overflow-hidden">
      {/* Page header */}
      <div className="bg-white border-b border-gray-200 px-3 py-1 text-[11px] font-bold text-black select-none text-center">
        Statistics
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 border-r border-gray-200 flex flex-col overflow-hidden">
                ...
                {vouchersRows.length === 0 ? (
                  ...
                  vouchersRows.map((row: any, idx: number) => {
                    ...
                    return (
                      <tr
                        key={row.vch_type}
                        className={`border-b border-gray-200 cursor-pointer select-none ${focused ? 'bg-black/[0.06] text-black font-bold' : 'hover:bg-black/[0.03] text-black'}`}
                        onClick={() => {
                          setActiveCol('vouchers');
          ...
          {vouchersRows.length > 0 && (
            <div className="bg-white border-t border-gray-200 font-bold text-black text-[11px] px-3 py-1.5 flex justify-between select-none">
              <span>Total</span>
