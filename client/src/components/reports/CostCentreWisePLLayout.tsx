...
export default function CostCentreWisePLLayout() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.report
      .run('cost_centre_wise_p_and_l', { company_id: cid, fy_id: fyid })
      .then((res: any) => {
        if (res?.success) {
          setRows(res.rows || []);
        } else {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="w-40 text-right px-4 py-1.5 border-l border-gray-200">
                      {fmt(row.expense_debit)}
                    </td>
                    <td className="w-40 text-right px-4 py-1.5 border-l border-gray-200">
                      {fmt(row.income_credit)}
                    </td>
                    <td
                      className={`w-40 text-right px-4 py-1.5 border-l border-gray-200 ${row.current_amount < 0 ? 'text-black' : ''}`}
                    >
          ...
          {rows.length > 0 && (
            <tfoot className="sticky bottom-0 bg-white border-t border-gray-200 z-10 font-bold text-black">
              <tr>
                ...
                <td
                  className={`w-40 text-right px-4 py-2 border-l border-gray-200 ${totalAmount < 0 ? 'text-black' : ''}`}
                >
