...
export default function MemorandumRegisterLayout() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.report
      .memorandumRegister(companyId, fyId)
      .then((res: any) => {
        if (res.success) {
          setMonthRows(res.rows || []);
          setFocusedMonthIndex(0);
        } else {
          setError(res.error || 'Failed to load Memorandum Register');
        }
      })
  ...
  const loadVouchersForMonth = React.useCallback(
    (monthRow: MonthRow) => {
      ...
      (window as any).api.report
        .memorandumRegisterVouchers(companyId, fyId, range.from, range.to)
        .then((res: any) => {
          if (res.success) {
            setVoucherRows(res.rows || []);
          } else {
            setError(res.error || 'Failed to load Memorandum vouchers');
          }
        })
        .catch((err: any) => setError(err.message))
          ...
          <table className="w-full border-collapse text-[11px] font-mono select-none">
            <thead className="sticky top-0 bg-white border-b border-gray-200 z-10 text-black">
              ...
              <tr className="bg-white">
                ...
                >
                  List of All Memorandum Vouchers — {selectedCompany?.name} — {selectedMonth.month}
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                voucherRows.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-1.5">{row.voucher_type || 'Memorandum'}</td>
