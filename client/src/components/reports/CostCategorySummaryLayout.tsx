...
export default function CostCategorySummaryLayout() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.report
      .costCategorySummary(selectedCompany.company_id, activeFY.fy_id)
      .then((res: any) => {
        if (res?.success) {
          setRows(res.rows || []);
        } else {
  ...
  React.useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (r) {
          navigate(
            `/reports/accounts/cost-centre-summary?category_name=${encodeURIComponent(r.category_name)}`,
          );
        }
              ...
              rows.map((row, idx) => {
                ...
                return (
                  <tr
                    ...
                    onDoubleClick={() =>
                      navigate(
                        `/reports/accounts/cost-centre-summary?category_name=${encodeURIComponent(row.category_name)}`,
                      )
                    }
                  >
