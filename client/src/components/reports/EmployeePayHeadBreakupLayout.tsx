...
export default function EmployeePayHeadBreakupLayout() {
  ...
  if (loading) return <div className="flex-1 flex items-center justify-center text-zinc-400 font-mono text-xs">Loading Employee Pay Head Breakup...</div>;
      ...
      <div className="flex-1 overflow-y-auto">
        <table className="w-full border-collapse text-[11px] font-mono select-none">
          <thead className="sticky top-0 bg-[#f4f4f5] border-b border-zinc-300 z-10">
            <tr>
              <th colSpan={3} className="px-3 py-0.5 text-left font-normal italic text-zinc-500">All employees — pay head breakup</th>
            ...
            {rows.length === 0 ? (
              ...
              <React.Fragment key={empName}>
                <tr className="bg-zinc-100 border-b border-zinc-200">
                  <td colSpan={4} className="px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-zinc-600">{empName}</td>
                </tr>
                {empRows.map((row) => {
