...
export function ReportRightPanel({
  ...
  return (
    <div className="w-28 bg-white border-l border-zinc-300 flex flex-col gap-0.5 p-1 select-none h-full shrink-0">
      {buttons.map((btn) => (
        <button
          key={btn.key}
          onClick={btn.action}
          disabled={btn.disabled}
          className="w-full text-left px-1.5 py-1 flex flex-col justify-center border border-zinc-300 rounded-sm bg-white hover:bg-zinc-100 active:bg-zinc-200 disabled:opacity-40 disabled:pointer-events-none transition-colors cursor-pointer"
        >
          <span className="font-bold text-[8px] leading-tight text-zinc-500">{btn.key}</span>
          <span className="font-semibold text-[9px] leading-tight line-clamp-1 mt-0.5 text-zinc-900">
            {btn.label}
          </span>
        </button>
      ))}
