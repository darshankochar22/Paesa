...
export default function ReversingJournalRegisterLayout() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.report
      .reversingJournalRegister(companyId, fyId)
      .then((res: any) => {
        if (res.success) {
          setMonthRows(res.rows || []);
          setFocusedMonthIndex(0);
        } else {
          setError(res.error || "Failed to load Reversing Journal Register");
        }
      })
  ...
  const loadVouchersForMonth = React.useCallback(
    (monthRow: MonthRow) => {
      ...
      (window as any).api.report
        .reversingJournalRegisterVouchers(companyId, fyId, range.from, range.to)
        .then((res: any) => {
          if (res.success) {
            setVoucherRows(res.rows || []);
          } else {
            setError(res.error || "Failed to load Reversing Journal vouchers");
          }
        })
        .catch((err: any) => setError(err.message))
  ...
  if (loadingMonths) {
    return (
      <div className="flex-1 flex items-center justify-center text-black/60 font-mono text-xs">
        Loading Reversing Journal Register...
      </div>
    );
  }
  if (error) {
  ...
  if (selectedMonth) {
    ...
    return (
      <div className="flex flex-col h-full min-h-full w-full bg-white font-mono overflow-hidden">
          ...
          <table className="w-full border-collapse text-[11px] font-mono select-none">
            <thead className="sticky top-0 bg-white border-b border-black z-10 text-black">
              ...
              <tr className="bg-white">
                <th colSpan={6} className="px-4 py-0.5 text-left font-normal italic text-black/60 border-b border-black/10">
                  List of All Reversing Journal Vouchers
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                voucherRows.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-1.5">{row.voucher_type || "Reversing Journal"}</td>
