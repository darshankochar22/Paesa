...
const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ className, options, placeholder, children, ...props }, ref) => (
    <select
      ref={ref}
      className={cn(
        "h-8 w-full min-w-0 px-2 pr-7 text-xs text-zinc-900 bg-white border border-zinc-300",
        "outline-none transition-colors hover:border-zinc-400 focus:border-zinc-800 cursor-pointer",
