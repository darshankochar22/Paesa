...
const Checkbox = React.forwardRef<HTMLInputElement, CheckboxProps>(
  ({ className, label, id, ...props }, ref) => {
    ...
    return (
      <label
        htmlFor={inputId}
        className="inline-flex items-center gap-2 cursor-pointer select-none text-xs text-zinc-900"
      >
        <input
          ref={ref}
          id={inputId}
          type="checkbox"
          className={cn(
            "h-3.5 w-3.5 appearance-none border border-zinc-400 bg-white cursor-pointer",
            "checked:bg-zinc-900 checked:border-zinc-900 transition-colors",
