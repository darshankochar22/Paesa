...
module.exports = {
  create: async (data) => {
    try {
      ...
      if (existing) return { success: false, error: 'Financial year already exists' };
      ...
      const fy = db.prepare(`SELECT * FROM financial_years WHERE fy_id = ?`).get(result.lastInsertRowid);
  ...
  getById: async (id) => {
    try {
      const fy = db.prepare(`SELECT * FROM financial_years WHERE fy_id = ?`).get(id);
  ...
  setActive: async (fy_id, company_id) => {
    try {
      const fy = db.prepare(`SELECT * FROM financial_years WHERE fy_id = ?`).get(fy_id);
      if (!fy) return { success: false, error: 'Financial year not found' };
  ...
  delete: async (id) => {
    try {
      ...
      db.prepare(`DELETE FROM financial_years WHERE fy_id = ?`).run(id);
      return { success: true };
    } catch (err) {
      return { success: false, error: err.message };
    }
