...
const seedDefaultGroups = (company_id) => {
  ...
  PRIMARY_GROUPS.forEach((g, i) => {
    ...
    seededPrimary.push({ id: result.lastInsertRowid, name: g.name });
  });
...
module.exports = {
  ...
  create: async (data) => {
    try {
      ...
      const group = db.prepare(`SELECT * FROM groups WHERE group_id = ?`).get(result.lastInsertRowid);
  ...
  getAll: async (company_id) => {
    try {
      const groups = db.prepare(`
        SELECT * FROM groups WHERE company_id = ? AND is_active = 1
      `).all(company_id);
  ...
  update: async (data) => {
    try {
      const group = db.prepare(`SELECT * FROM groups WHERE group_id = ?`).get(data.group_id);
      if (!group) return { success: false, error: 'Group not found' };
      ...
      const updated = db.prepare(`SELECT * FROM groups WHERE group_id = ?`).get(data.group_id);
  ...
  delete: async (id) => {
    try {
      ...
      const hasChildren = db.prepare(`
        SELECT * FROM groups WHERE parent_group_id = ? AND is_active = 1
      `).get(id);
      if (hasChildren) return { success: false, error: 'Cannot delete group with subgroups' };

      db.prepare(`UPDATE groups SET is_active = 0 WHERE group_id = ?`).run(id);
      return { success: true };
    } catch (err) {
      return { success: false, error: err.message };
    }
