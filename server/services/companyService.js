...
module.exports = {
  create: async (data) => {
    try {
      ...
      companyCreationSuccessService.seedCompanyCreationSuccess(
        company.company_id,
      );
  ...
  getAll: async () => {
    try {
      const companies = db.execute(`SELECT * FROM companies`).all();
  ...
  delete: async (id) => {
    try {
      ...
      db.execute(`DELETE FROM companies WHERE company_id = ?`).run(id);
      return { success: true };
    } catch (err) {
      return { success: false, error: err.message };
    }
