...
module.exports = {
    ...
    try {
      ...
      if (data.bank_details) {
        ...
        `).run({
          ...
          enable_cheque_printing:        data.bank_details.enable_cheque_printing ? 1 : 0,
      ...
      const ledger = db.prepare(`SELECT * FROM ledgers WHERE ledger_id = ?`).get(ledger_id);
  ...
  getById: async (id) => {
    try {
      const ledger = db.prepare(`SELECT * FROM ledgers WHERE ledger_id = ?`).get(id);
      if (!ledger) return { success: false, error: 'Ledger not found' };

      const bank_details      = db.prepare(`SELECT * FROM ledger_bank_details WHERE ledger_id = ?`).get(id) || null;
      const statutory_details = db.prepare(`SELECT * FROM ledger_statutory_details WHERE ledger_id = ?`).get(id) || null;

      return { success: true, ledger: { ...ledger, bank_details, statutory_details } };
    } catch (err) {
      return { success: false, error: err.message };
    }
  },
  ...
  update: async (data) => {
    try {
      const ledger = db.prepare(`SELECT * FROM ledgers WHERE ledger_id = ?`).get(data.ledger_id);
      if (!ledger) return { success: false, error: 'Ledger not found' };
      ...
      if (data.bank_details) {
        const existing = db.prepare(`SELECT * FROM ledger_bank_details WHERE ledger_id = ?`).get(data.ledger_id);
        if (existing) {
          db.prepare(`
      ...
      if (data.statutory_details) {
        const existing = db.prepare(`SELECT * FROM ledger_statutory_details WHERE ledger_id = ?`).get(data.ledger_id);
        if (existing) {
          db.prepare(`
      ...
      const updated = db.prepare(`SELECT * FROM ledgers WHERE ledger_id = ?`).get(data.ledger_id);
  ...
  delete: async (id) => {
    try {
      ...
      db.prepare(`UPDATE ledgers SET is_active = 0 WHERE ledger_id = ?`).run(id);
      return { success: true };
    } catch (err) {
      return { success: false, error: err.message };
    }
