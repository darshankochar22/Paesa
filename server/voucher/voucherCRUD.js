...
// Read-side queries (getAll/getById/getDaybook/getByType/getByLedger + attendance
// row mapping) live in voucherReads.js — this module re-exports them to keep the
// public voucherCRUD API unchanged for all existing call sites.
const voucherReads = require('./voucherReads');
// Write path (update/cancel/delete) lives in voucherUpdate.js; the GST-class and
// non-accounting helpers it shares with create() live in voucherCommon.js. All are
// re-exported here so the public voucherCRUD API stays unchanged for every call site.
const {
