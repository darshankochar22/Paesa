...
const generateAnnualComputation = async (company_id, fy_id) => {
  try {
    ...
    } catch (_) {/* financial_years table may not exist yet; proceed with fallback */}
    ...
    for (const voucher of rawVouchers) {
        ...
        try {
          ...
          computed.stock_entries.forEach(e => {
            ...
            if (computed.is_inter_state) {
              if (e.gst_rate > 0)
                taxLines.push({ hsnCode: hsn, assessableValue: e.assessable_value, taxType: 'IGST', rate: e.gst_rate, amount: e.igst_amount });
            } else {
              if (e.gst_rate > 0) {
                taxLines.push({ hsnCode: hsn, assessableValue: e.assessable_value, taxType: 'CGST', rate: e.gst_rate / 2, amount: e.cgst_amount });
                taxLines.push({ hsnCode: hsn, assessableValue: e.assessable_value, taxType: 'SGST', rate: e.gst_rate / 2, amount: e.sgst_amount });
              }
            }
            if (e.cess_amount > 0)
              taxLines.push({ hsnCode: hsn, assessableValue: e.assessable_value, taxType: 'CESS', rate: 0, amount: e.cess_amount });
          });
        } catch (err) {
      ...
      let txval = 0, iamt = 0, camt = 0, samt = 0, cess = 0;
      const seen = new Set();
      ...
      let isRc = false, isExempt = false, isNonGst = false;
      const classId = taxLines.length > 0
      ...
      if (isOutward) {
        const isExport = voucher.place_of_supply === 'Export'
          || partyStateCode === '97' || partyStateCode === '96';
      ...
      if (isInward) {
        ...
        const isImport = partyStateCode === '97' || partyStateCode === '96'
          || voucher.place_of_supply === 'Import';
        const isService = stockEntries.length > 0
    ...
    [outward_taxable, outward_zero, outward_nil_exmp, outward_nongst, inward_rcm,
     itc_import_goods, itc_import_services, itc_rcm, itc_other, itc_reversed].forEach(round2);
    round2(tax_payable);
    round2(tax_paid);

    // Total ITC available
    const total_itc_availed = {
      iamt: Number((itc_import_goods.iamt + itc_import_services.iamt + itc_rcm.iamt + itc_other.iamt).toFixed(2)),
      camt: Number((itc_import_goods.camt + itc_import_services.camt + itc_rcm.camt + itc_other.camt).toFixed(2)),
      samt: Number((itc_import_goods.samt + itc_import_services.samt + itc_rcm.samt + itc_other.samt).toFixed(2)),
      cess: Number((itc_import_goods.cess + itc_import_services.cess + itc_rcm.cess + itc_other.cess).toFixed(2)),
    };

    // Net tax payable
    const net_tax = {
      igst: Number(Math.max(0, tax_payable.igst - total_itc_availed.iamt).toFixed(2)),
      cgst: Number(Math.max(0, tax_payable.cgst - total_itc_availed.camt).toFixed(2)),
      sgst: Number(Math.max(0, tax_payable.sgst - total_itc_availed.samt).toFixed(2)),
      cess: Number(Math.max(0, tax_payable.cess - total_itc_availed.cess).toFixed(2)),
    };
