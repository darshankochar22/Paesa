...
const profitLoss = async (company_id, fy_id, from_date = null, to_date = null) => {
  try {
    ...
    const totalIndirectExpenses = Math.abs(
      buckets.indirectExpenses.reduce((s, g) => s + g.balance, 0),
    );
    ...
    const totalIndirectIncomes = Math.abs(
      buckets.indirectIncomes.reduce((s, g) => s + g.balance, 0),
    );
    ...
    // F11 "Integrate Accounts with Inventory": only when ON does inventory
    // valuation flow into the trading account. OFF = accounts and inventory are
    // kept separate (Tally), so opening/closing stock stay 0 here and the user
    // records stock via a manual Stock-in-Hand ledger instead.
    ...
    try {
      ...
      /* no stock_items table or no data */
    }
