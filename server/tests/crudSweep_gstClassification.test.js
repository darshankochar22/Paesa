...
describe('GST Classification CRUD sweep (UI-faithful)', () => {
  ...
  // ----- Predefined classifications: can't be edited, but CAN be deleted -----
  // Companies no longer auto-seed predefined classifications, so seed them
  // explicitly here to exercise the guard behaviour on predefined rows.
  it('refuses to UPDATE a predefined classification but allows DELETE', async () => {
    await gstClassificationService.seedDefaultGSTClassifications(companyId);
    const all = await gstClassificationController.getAll(null, companyId);
    const predefined = all.gstClassifications.find((x) => x.is_predefined === 1);
    expect(predefined).toBeDefined();

    // Editing a predefined row stays blocked.
    ...
    // Deleting a predefined row now succeeds (user removes seeded slabs).
    const del = await gstClassificationController.delete(null, predefined.gc_id);
    expect(del.success).toBe(true);

    // Soft-deleted: gone from the active list.
    const after = await gstClassificationController.getAll(null, companyId);
    expect(after.gstClassifications.some((x) => x.gc_id === predefined.gc_id)).toBe(false);
  });
