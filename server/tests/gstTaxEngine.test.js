...
const taxLinesOf = async (voucherId) =>
  (
    await db.execute(`SELECT tax_type, amount FROM gst_voucher_tax_lines WHERE voucher_id = ?`, [
      voucherId,
    ])
  ).rows;
...
describe('GST tax engine — manual tax ledger model', () => {
  ...
  beforeAll(async () => {
    ...
    const partyKA = await ledgerService.create({
      company_id: companyId,
      name: 'Manual Customer KA',
      gstin: '29ZZZZZ1234F1Z5',
      state: 'Karnataka',
      country: 'India',
      registration_type: 'Regular',
    });
    ...
    salesId = ledgerId(
      await ledgerService.create({ company_id: companyId, name: 'Manual Sales A/c' }),
    );
  ...
  const createVoucher = (extra) =>
    voucherController.create(null, {
      ...
      date: '2026-04-10',
      status: 'Regular',
      reference_number: `INV-${Date.now()}-${Math.random()}`,
  ...
  it('BUG 2: manually selected CGST+SGST are saved AS-IS on an intra-state supply, not swapped to IGST', async () => {
    ...
    expect(entries.find((e) => Number(e.ledger_id) === Number(igstId))).toBeFalsy();
    expect((await taxLinesOf(res.voucher.voucher_id)).map((r) => r.tax_type).sort()).toEqual([
      'CGST',
      'SGST',
    ]);
  });
  ...
  it('BUG 3: inter-state supply blocks CGST/SGST and accepts a single IGST', async () => {
    ...
    const blocked = await createVoucher({
      party_ledger_id: partyKAId,
      party_name: 'Manual Customer KA',
      place_of_supply: 'Karnataka',
      entries: [
        {
          ledger_id: partyKAId,
          ledger_name: 'Manual Customer KA',
          type: 'Dr',
          amount: 11800,
          currency: 'INR',
        },
  ...
  it('BUG 4: blocks a GST ledger when the item is Exempt / Nil Rated', async () => {
    const res = await createVoucher({
      ...
      stock_entries: [
        { stock_item_id: exemptItemId, item_name: 'Exempt Widget', quantity: 1, rate: 1000 },
      ],
    });
    expect(res.success).toBe(false);
  ...
  it("inter-state multi-item invoice posts exactly ONE IGST = sum of per-item 18% (user's 9594 case)", async () => {
    ...
    expect(res.success).toBe(true);
    const igstEntries = (await entriesOf(res.voucher.voucher_id)).filter(
      (e) => Number(e.ledger_id) === Number(igstId),
    );
...
describe('GST tax engine — Voucher Type Class ledger override (opt-in auto-inject)', () => {
  ...
  beforeAll(async () => {
    ...
    classCgstId = ledgerId(
      await ledgerService.create({
        company_id: companyId,
        name: 'Class-Mapped CGST',
        group_id: dutiesGroupId,
        statutory_details: { type_of_duty_tax: 'GST', gst_tax_type: 'CGST', gst_rate: 9 },
      }),
    );
    classSgstId = ledgerId(
      await ledgerService.create({
        company_id: companyId,
        name: 'Class-Mapped SGST',
        group_id: dutiesGroupId,
  ...
  const createSalesVoucher = (voucher_class) =>
    voucherController.create(null, {
      company_id: companyId,
      fy_id: fyId,
      voucher_type: 'Sales',
      date: '2026-04-10',
      status: 'Regular',
      reference_number: `INV-CLS-${Date.now()}-${Math.random()}`,
      place_of_supply: 'Maharashtra',
      party_ledger_id: partyId,
      party_name: 'Voucher Class Customer',
      is_accounting_voucher: 1,
      is_invoice: 1,
      is_inventory_voucher: 1,
      ...
      entries: [
        {
          ledger_id: partyId,
          ledger_name: 'Voucher Class Customer',
          type: 'Dr',
          amount: 11800,
          currency: 'INR',
        },
        {
          ledger_id: salesId,
          ledger_name: 'Voucher Class Sales A/c',
          type: 'Cr',
          amount: 11800,
          currency: 'INR',
        },
...
// Data-layer invariant behind gstValidation.validateGstLedgers: the STORED tax lines of a
// voucher can never mix sides, whatever code path writes them (both save fns call this).
describe('GST tax engine — assertGstSidesExclusive invariant', () => {
  const { assertGstSidesExclusive } = require('../gst/gstTaxEngine');

  it('accepts an intra-state pair (CGST + SGST, no IGST)', () => {
    expect(() =>
      assertGstSidesExclusive(
        [
          { tax_type: 'CGST', amount: 900 },
          { tax_type: 'SGST', amount: 900 },
        ],
        1,
      ),
    ).not.toThrow();
  ...
  it('accepts an inter-state line (IGST only)', () => {
    expect(() => assertGstSidesExclusive([{ tax_type: 'IGST', amount: 1800 }], 2)).not.toThrow();
  });

  it('throws when a voucher mixes IGST with CGST/SGST', () => {
    expect(() =>
      assertGstSidesExclusive(
        [
          { tax_type: 'IGST', amount: 1800 },
          { tax_type: 'CGST', amount: 900 },
        ],
        3,
      ),
    ).toThrow(/both IGST and CGST\/SGST/);
  });

  it('ignores zero-amount lines (a 0-amount IGST line alongside CGST/SGST is not a mix)', () => {
    expect(() =>
      assertGstSidesExclusive(
        [
